...
(function () {
  ...
  function _replaySecurityChecks(op, done) {
    ...
    if (op.type === 'joinMatch' && window.U) {
      ...
      if (window._supa) {
        ...
        window._supa.from('users').select('is_banned').eq('id', uid).single()
          .then(function(r) {
  ...
  function _executeOp(op, done) {
    switch (op.type) {
      ...
      case 'joinMatch':
        var p = op.payload;
        /* Bug 4 Fix: Run security checks first */
        _replaySecurityChecks(op, function (ok, reason) {
          if (!ok) {
            if (window.toast) window.toast('⚠️ Queued join rejected: ' + (reason || 'Security check failed'), 'err');
            done(true); /* Mark as done (not retried) */
            return;
          }
          /* Bug 7 Fix: Check slot availability before replaying paid join */
          if (window._supa && p.entryFee > 0) {
            function _doRPCReplay() {
              window._supa.rpc('validate_and_join_match', {
                p_uid:       window.U && window.U.uid,
                ...
                if (r && r.data && r.data.ok === false) {
                  var errMsg = r.data.error || 'Server rejected';
                  if (window.toast) window.toast('❌ Queued join failed: ' + errMsg, 'err');
                  done(true);
                } else {
                  window.db && window.db.ref('joinRequests').push(Object.assign({}, p, {
            ...
            /* Pre-check slots before deducting fee */
            window._supa.from('matches').select('data').eq('firebase_id', p.matchId).single()
              .then(function(mr) {
                ...
                if (filled >= maxS) {
                  ...
                  done(true); return; // discard, don't deduct
                }
                _doRPCReplay();
              }).catch(function() { _doRPCReplay(); });
          /* Free match (entryFee = 0) — Bug Critical #3 Fix:
             Also insert into Supabase join_requests (user panel reads from Supabase).
             Previous code only wrote to Firebase joinRequests/ which user panel ignores. */
          var _freeJoinPayload = Object.assign({}, p, {
            timestamp: firebase.database.ServerValue.TIMESTAMP,
            status: 'joined', _offlineQueued: true
          });
          // Firebase mirror (for admin panel compatibility)
          if (window.db) {
            window.db.ref('joinRequests').push(_freeJoinPayload)
              .then(function() { done(true); })
              .catch(function() { done(false); });
          }
          // Supabase (source of truth for user panel)
          if (window._supa && window.U) {
            ...
            }).catch(function(e){ console.warn('[OQ] Supabase free join insert fail:', e.message); });
          }
        });
        break;
