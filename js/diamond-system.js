    ...
    db.ref('appSettings/withdrawal').once('value', function (cfgSnap) {
      ...
      if (window._supa) {
        /* ✅ SECURITY FIX (2026-09-08): "server-authoritative wallet
           RPC" audit — replaced the old decrement_balance-then-insert-
           then-refund-on-failure chain with ONE atomic RPC
           (submit_gd_withdrawal) that deducts and records the request
           in a single database transaction. The old pattern's
           refund-on-failure step called increment_balance directly
           from the client, which is being locked to service_role-only
           this session; that step would have started silently failing
           (leaving diamonds gone with no request on file) the moment
           that lockdown shipped. A real transaction has no such
           window — either both the deduction and the request row
           happen, or neither does. */
        window._supa.rpc('submit_gd_withdrawal', {
          p_gd_amount: diaAmt, p_amount_inr: rupeesGot, p_upi: upi,
          p_notes: 'GD withdrawal | TDS: ₹' + tdsAmt + ' | FY: ' + fy
        }).then(function(res) {
            var d = res && res.data;
            if (!d || !d.success) {
              var msg = (d && d.error) || 'Balance deduct nahi hua';
              console.error('[DiamondSystem] GD withdrawal failed:', msg);
            ...
            _reqId = d.requestId;
              ...
              if (window.UD) {
                window.UD.green_diamonds = d.newBalance;
                window.UD.greenDiamonds = d.newBalance;
              }
              if (window.updateHdr) updateHdr();
            ...
            console.error('[DiamondSystem] submit_gd_withdrawal RPC error:', err);
            if (window.toast) toast('❌ Withdrawal fail — try again', 'err');
          });
      }
    });
  };
