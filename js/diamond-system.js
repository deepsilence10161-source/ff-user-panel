...
(function () {
  ...
  /* ════ OVERRIDE: renderWallet — Sky Diamond + Coin display ════ */
  ...
  window.renderWallet = function () {
    ...
    /* Always show walletMain */
    var wm = document.getElementById('walletMain'), wf = document.getElementById('walletFlow');
    ...
    if (wb) {
      ...
      if (_tdsActive) {
        /* DISABLED: Real money withdrawal removed — halal model */ // breakText += '...';
      } else {
        /* DISABLED: Real money withdrawal removed — halal model */ // breakText += '...';
      }
  ...
  /* ════ OVERRIDE: updateHdr — Sky Diamond in header ════ */
  var _origUpdateHdr = window.updateHdr;
  window.updateHdr = function () {
    if (!window.UD) { if (_origUpdateHdr) _origUpdateHdr(); return; }
    var rm = window.UD.realMoney || {};
    /* Sky Diamonds = deposited only (what user bought) */
    var skyDia = Math.max(Number(rm.deposited)||0, 0);
    var coins  = Math.max(Number(window.UD.coins)||0, 0);
    var hc = document.getElementById('hdrCoins');
    var hm = document.getElementById('hdrMoney');
    if (hc) hc.textContent = coins;
    if (hm) {
      hm.textContent = skyDia;
      var chip = hm.parentElement;
      if (chip) {
        var iconSpan = chip.querySelector('span:first-child');
        if (iconSpan) iconSpan.textContent = DIAMOND_SYMBOL;
        chip.style.background = 'rgba(0,212,255,.12)';
        chip.style.color = '#00d4ff';
      }
    }
    /* ✅ BUG FIX (2026-08-25): this override replaced updateHdr()
       entirely for Sky Diamond/coins purposes, but never touched the
       Green Diamond chip (#hdrGD) at all — and never fell back to the
       original updateHdr() to cover it either (the fallback only ran
       when window.UD was missing entirely). Every single call to
       updateHdr() anywhere in the app therefore silently skipped the
       Green Diamond chip forever after this override installed, so it
       stayed frozen at its initial "0" from the raw HTML no matter what
       the real balance was — confirmed live: wallet screen showed 40
       Green Diamonds correctly (reads the same window.UD.greenDiamonds
       field) while the header chip stayed stuck at 0. Restoring the
       Green Diamond chip update here, matching core/header.js's logic
       exactly, so both header fields get updated on every call again. */
    var hgd = document.getElementById('hdrGD');
    if (hgd) hgd.textContent = Math.max(Number(window.UD.greenDiamonds) || 0, 0);
  };
  ...
  function addDiamondConversionCard() {
    ...
    /* TDS badge hidden */ //h += '<div style="font-size:10px;background:' + bgCol + ';border:1px solid ' + borderCol + ';color:' + titleCol + ';padding:3px 8px;border-radius:20px">' + (_tdsActive ? 'TDS ON' : 'TDS OFF') + '</div>';
    h += '</div>';
    ...
    if (firstCard && firstCard.nextSibling) {
      try{walletMain.insertBefore(card, firstCard.nextSibling);}catch(e){}
    } else {
  ...
  window.startWd = function () {
    ...
    h += '<div class="f-group"><label>Payment Screenshot (UPI screenshot - optional but recommended)</label>';
    h += '<div id="_wdSsArea" onclick="document.getElementById(\'_wdSsIn\').click()" style="border:2px dashed rgba(255,255,255,.15);border-radius:12px;padding:16px;text-align:center;cursor:pointer;background:rgba(255,255,255,.02)">';
    h += '<i class="fas fa-camera" style="font-size:24px;color:#555;display:block;margin-bottom:6px"></i>';
    h += '<div style="font-size:12px;color:#555">Tap to add screenshot (optional)</div>';
    h += '<input type="file" id="_wdSsIn" accept="image/*" style="display:none" onchange="window._wdHandleSs(this)">';
    h += '</div><img id="_wdSsPreview" style="display:none;width:100%;border-radius:10px;margin-top:8px"></div>';
  ...
  /* Screenshot handler for withdrawal */
  var _wdScreenshot = '';
  window._wdHandleSs = function(inp) {
    ...
    reader.onload = function(e) {
      ...
      img.onload = function() {
        ...
        var q = 0.7, data = cv.toDataURL('image/jpeg', q);
        while (data.length > 400*1024 && q > 0.2) { q -= 0.1; data = cv.toDataURL('image/jpeg', q); }
        _wdScreenshot = data;
        var prev = document.getElementById('_wdSsPreview');
        var area = document.getElementById('_wdSsArea');
        if (prev) { prev.src = data; prev.style.display = 'block'; }
        if (area) area.innerHTML = '<i class="fas fa-check-circle" style="color:#00ff9c;font-size:20px;margin-bottom:4px;display:block"></i><div style="font-size:11px;color:#00ff9c">Screenshot added ✅</div><input type="file" id="_wdSsIn" accept="image/*" style="display:none" onchange="window._wdHandleSs(this)">';
      };
  ...
  window._confirmDiamondWD = function (maxDiamonds) {
    ...
    db.ref('appSettings/withdrawal').once('value', function (cfgSnap) {
      ...
      /* Bug #27 Fix: Write to Supabase sd_requests instead of Firebase walletRequests */
      var _reqId = 'WD_' + U.uid.substring(0,8) + '_' + Date.now();
      if (window._supa) {
        /* ✅ BUG FIX (2026-07): previously this called increment_balance
           with a NEGATIVE p_amount to "deduct" green_diamonds —
           increment_balance explicitly rejects negative amounts
           (RAISE EXCEPTION 'Amount must be non-negative'), so this call
           has never once succeeded; it silently threw and was swallowed
           by .catch(), meaning a withdrawal request row got created but
           the user's green_diamonds balance was NEVER actually deducted,
           for every withdrawal request ever submitted through this flow.
           decrement_balance (p_uid, p_col, POSITIVE p_amount) is the
           correct, already-existing, balance-checked function for this —
           it's used the same way elsewhere in the app (e.g.
           features/squad-bank.js). Also reordered: deduct FIRST, only
           record the withdrawal request if the deduction actually
           succeeds, so a race condition (balance changed between the
           screen loading and the user submitting) can't create a request
           for diamonds the user no longer has. */
        window._supa.rpc('decrement_balance', { p_uid: U.uid, p_col: 'green_diamonds', p_amount: diaAmt })
          .then(function(res) {
            if (!res || res.error || (res.data && res.data.success === false)) {
              var msg = (res && res.data && res.data.error) || (res && res.error && res.error.message) || 'Balance deduct nahi hua';
              console.error('[DiamondSystem] GD deduct failed:', msg);
              if (window.toast) toast('❌ Withdrawal fail: ' + msg, 'err');
              return;
            }

            /* Deduction confirmed — now safe to record the request. */
            window._supa.from('sd_requests').insert({
              /* ✅ BUG FIX (2026-07): sd_requests.id is a UUID PRIMARY KEY —
                 inserting a non-UUID string like _reqId here would fail
                 outright ("invalid input syntax for type uuid"). Let Postgres
                 auto-generate the real id; _reqId is still used below as a
                 human-readable tag in wallet_transactions.ref_id (a plain
                 TEXT column, no format requirement there). */
              user_id: U.uid,
              ign: UD.ign || UD.displayName || '',
              sd_amount: diaAmt,
              amount_inr: rupeesGot,
              screenshot_url: null,
              upi_ref: upi,
              status: 'pending',
              notes: 'GD withdrawal | TDS: ₹' + tdsAmt + ' | FY: ' + fy,
              request_type: 'green_diamond_withdrawal'
            }).then(function(insRes) {
              if (insRes && insRes.error) {
                /* Deduction succeeded but the request record failed to
                   save — this is now a genuine "money taken, no request
                   on file" state. Credit it straight back rather than
                   leaving the user's diamonds gone with no pending
                   request to show for it. */
                console.error('[DiamondSystem] Withdrawal request save failed AFTER deduction — refunding:', insRes.error.message);
                window._supa.rpc('increment_balance', { p_uid: U.uid, p_col: 'green_diamonds', p_amount: diaAmt }).then(null, function(){});
                if (window.toast) toast('❌ Request save fail hua, diamonds wapas kar diye', 'err');
                return;
              }

              /* ✅ BUG FIX (2026-07): everything below (TDS records,
                 transaction log, notification, closeModal, success toast)
                 previously ran synchronously and unconditionally right
                 after the withdrawal request insert was FIRED (not
                 awaited) — meaning the user saw a success toast and the
                 modal closed even on requests where the deduction or the
                 request-save had already failed above. Moved inside this
                 success callback so the user only sees success confirmation
                 once the balance has actually been deducted and the
                 request has actually been saved. */
              /* ✅ BUG FIX (2026-08-25): only green_diamonds (snake_case)
                 was updated — header.js/wallet.js read greenDiamonds
                 (camelCase), so updateHdr() right after still rendered
                 the stale pre-withdrawal value. */
              if (window.UD) {
                var _gdAfter = (Number(window.UD.green_diamonds) || 0) - diaAmt;
                window.UD.green_diamonds = _gdAfter;
                window.UD.greenDiamonds = _gdAfter;
              }
              if (window.updateHdr) updateHdr();
              ...
              if (_tdsActive && tdsAmt > 0) {
                ...
                db.ref('tdsRecords').push({
                  ...
                  /* Was 'id', an undefined variable in this scope — see
                     full explanation in the earlier comment block. Fixed
                     to _reqId, the actual request identifier in scope. */
                  upiId: upi, walletRequestId: _reqId,
                  financialYear: fy, timestamp: Date.now()
                });
                db.ref('tdsHeld').push({
                  uid: U.uid, ign: UD.ign || UD.displayName || '',
                  amount: tdsAmt, walletRequestId: _reqId,
            ...
            window._supa.from('wallet_transactions').insert({
              user_id: U.uid, txn_type: 'debit', amount: diaAmt,
              currency: 'green_diamonds',
              reason: '💸 GD withdrawal request — ₹' + rupeesGot + ' pending',
              ref_id: _reqId
            }).then(null, function(){});
          });
      }
