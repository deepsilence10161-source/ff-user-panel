...
waitFor(function(){return window.U&&window._supa;}, function(){
  ...
  window._checkStreakFixed = function(){
    ...
    window._supa.from('users').select('last_checkin_date,streak_days').eq('id',uid).single().then(function(r){
      if (r.error) return;
      var data = r.data || {};
      var last = data.last_checkin_date || '', streak = Number(data.streak_days) || 0;
      if (last === today) return; /* already credited today — via this path OR the manual button */
      var newStreak;
      ...
      var cycleStreak = ((newStreak - 1) % 7) + 1; /* 1-7 reward tier, repeating */
      var REWARDS = window._adminDailyBonusRewards || [5,7,10,12,15,20,30];
      var reward = REWARDS[cycleStreak - 1] || 5;
      var bonus = (newStreak % 30 === 0) ? 100 : 0;
      window._supa.from('users').update({ last_checkin_date: today, streak_days: newStreak }).eq('id', uid)
        .then(function(upd){
          if (upd.error) throw upd.error;
          return window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'coins', p_amount: reward + bonus });
        })
        .then(function(){
          if (window.UD) window.UD.coins = (Number(window.UD.coins) || 0) + reward + bonus;
          if (window.updateHdr) updateHdr();
          window._supa.from('daily_checkins').upsert({
            user_id: uid, checkin_date: today, coins_earned: reward + bonus, streak_day: newStreak
