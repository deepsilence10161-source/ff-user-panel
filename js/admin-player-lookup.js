...
function searchPlayers() {
    ...
    usersSnapshot.forEach(function(c) {
        var u = c.val();
        var uid = c.key;
        /* L3 Fix: Normalise query and FF UID to same case — FF UIDs are uppercase
           by convention but users often type them lowercase. */
        var queryLower = query.toLowerCase();
        var queryUpper = query.toUpperCase();
        var matches =
            (u.ign   && u.ign.toLowerCase().includes(queryLower))  ||
            uid.toLowerCase().includes(queryLower)                  ||
            (u.ffUid && u.ffUid.toUpperCase().includes(queryUpper)) ||
