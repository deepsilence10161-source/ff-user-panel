...
(function () {
  ...
  function patchSelfExclusion() {
    ...
    // Override mesCheckExclusion to do async DB check
    window.mesCheckExclusionAsync = function (cb) {
      var U = window.U;
      var db = window.db;
      if (!U || !db) { cb(false); return; }
    ...
    function doLock() {
      ...
      if (UD && UD.referredBy) {
        ...
        if (!lockMsg) {
          ...
          inp.parentNode.insertBefore(lockMsg, inp.nextSibling);
        }
      }
    }
  ...
  /* ─────────────────────────────────────────────
     APPLY ALL PATCHES
     Wait for app.js to fully load + doJoin to exist
     ──────────────────────────────────────────── */
  function applyAll() {
    // Apply in order — each patch wraps the previous doJoin
    applyJoinRateLimit();
    patchOfflineJoin();      // outermost offline check
    patchSelfExclusion();    // inner exclusion check
    lockReferralUI();
