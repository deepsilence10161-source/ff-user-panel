...
window._showAchievementsLegacy = function () {
  ...
  */
  ...
  window.doCheckIn = function () {
    ...
    var uid = _safeUid(); if (!uid) return;
    var todayStr = new Date().toISOString().split('T')[0]; /* YYYY-MM-DD, matches DATE column */
    var btn = document.getElementById('checkInBtn');
    if (btn) btn.disabled = true;

    window._supa.from('users').select('last_checkin_date,streak_days,coins').eq('id', uid).single()
      .then(function(r) {
        ...
        /* Was it yesterday? Continue streak; otherwise reset to day 1 */
        var y = new Date(); y.setDate(y.getDate() - 1);
        var yStr = y.toISOString().split('T')[0];
        var newStreak = (row.last_checkin_date === yStr) ? (Number(row.streak_days) || 0) + 1 : 1;
        var reward = (window.CFG && window.CFG.checkinCoins) || 5;
        var streakBonus = (newStreak % 7 === 0) ? ((window.CFG && window.CFG.checkinStreakBonus7) || 50) : 0;
        ...
        .then(function(upd) {
          ...
          return window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'coins', p_amount: totalReward });
        })
        .then(function(rpcRes) {
          if (rpcRes && rpcRes.error) throw rpcRes.error;
          /* Audit log — best effort, doesn't block the reward */
          window._supa.from('daily_checkins').upsert({
            user_id: uid, checkin_date: todayStr, coins_earned: totalReward, streak_day: newStreak
          }, { onConflict: 'user_id,checkin_date' }).catch(function(){});

          /* ✅ Refresh local state + header so the new coin count is visible immediately */
          if (window.UD) window.UD.coins = (Number(window.UD.coins) || 0) + totalReward;
          if (window.updateHdr) updateHdr();
          if (window.renderWallet) renderWallet();

          _toast(streakBonus
            ? '🎉 Check-in complete! +🪙' + totalReward + ' Coins (7-day streak bonus!) 🔥'
      ...
      .catch(function(e) {
        console.error('[doCheckIn] read failed:', e && e.message);
        _toast('Check-in failed — retry karo', 'err');
        if (btn) btn.disabled = false;
      });
  };
