...
window._showAchievementsLegacy = function () {
    var UD = window.UD; if (!UD) return;
    var st = UD.stats || {};
    var achievements = [
      { title: 'First Blood 🩸', desc: 'Pehla match khela!', earned: (st.matches || 0) >= 1, icon: '🩸' },
  ...
  */
  ...
  window.showOpponentPreview = function (ffUid) {
    db.ref('users').orderByChild('ffUid').equalTo(ffUid).once('value', function (s) {
      ...
      h += '<div style="font-size:18px;font-weight:900">' + (window.escHtml?window.escHtml(u.ign||'Player'):(u.ign||'Player')) + '</div>';
      h += '<div style="font-size:11px;color:var(--txt2)">FF UID: ' + (u.ffUid||'—') + '</div>';
      if (u.bio) h += '<div style="font-size:12px;font-style:italic;color:var(--green);margin-top:6px">"' + (window.escHtml?window.escHtml(u.bio):u.bio) + '"</div>';
  ...
  window.checkMaintenance = function () {
    /* ✅ MIGRATED (2026-08-18): Maintenance Mode moved from Firebase RTDB
       ...
       Realtime (postgres_changes) for the live-toggle behavior the old
       Firebase .on('value') listener provided. */
    ...
    window._supa.channel('app_settings_maintenance_user')
      .on('postgres_changes', {event:'UPDATE', schema:'public', table:'app_settings', filter:'key=eq.maintenance'}, function (payload) {
        applyMaintState(!!(payload.new && payload.new.value && payload.new.value.active === true));
      }).subscribe();
  };
  ...
  window.showProfileWizard = function () {
    var UD = window.UD; if (!UD) return;
    /* L5 Fix: Don't show wizard if profile is already approved — user has nothing to do */
    var isApproved = UD.profileStatus === 'approved' || UD.profileVerified === true;
    if (isApproved && UD.ign && UD.ffUid) {
      _toast('✅ Profile already verified hai!', 'ok'); return;
    }
    var steps = [];
  ...
  window.showShareOptions = function (text, title) {
    var rawText = text || '';
    var encoded = encodeURIComponent(rawText);
    /* ✅ BUG FIX (2026-08-23): WhatsApp entry now uses the intent://
       hand-off (see core/utils.js _waShareUrl) instead of a plain
       wa.me link — the app's native WebView ad SDK was rewriting plain
       https://wa.me/ clicks into a broken whatsapp://...&type=
       custom_url&app_absent=0 intent that nothing on the device
       resolves. Telegram isn't affected by this (different SDK
       behavior), so it's left as a plain link. */
    var h = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:8px">';
    [
      ['WhatsApp', 'fab fa-whatsapp', '#25d366', window._waShareUrl ? window._waShareUrl(rawText) : ('https://wa.me/?text=' + encoded)],
  ...
  window.showMatchHistoryDetailed = function() {
    ...
    h += '<div style="font-size:14px;font-weight:800">' + (window.escHtml?window.escHtml(UD&&UD.ign?UD.ign:'Player'):(UD&&UD.ign?UD.ign:'Player')) + '</div>';
  ...
  window.validateMyFFUID = function() {
    ...
    if (UD.ffUid && UD.ign) {
      ...
      h += '<div style="font-size:16px;font-weight:800;margin-bottom:4px">' + (window.escHtml?window.escHtml(UD.ign):UD.ign) + '</div>';
  ...
  window.showTeamUIDs = function() {
    ...
    h += '<div style="font-size:14px;font-weight:800">' + (window.escHtml?window.escHtml(UD.ign||'You'):(UD.ign||'You')) + '</div>';
  ...
  window.showProfileSummary = function() {
    ...
    h += '<div style="font-size:18px;font-weight:800">' + (window.escHtml?window.escHtml(UD.ign||'Player'):(UD.ign||'Player')) + '</div>';
  ...
  window.showKillLeaderboard = function() {
    ...
    db.ref('users').orderByChild('stats/kills').limitToLast(20).once('value', function(s) {
      ...
      users.forEach(function(u, i) {
        ...
        h += '<div style="flex:1"><div style="font-size:13px;font-weight:700">' + (window.escHtml?window.escHtml(u.ign||u.displayName||'Player'):(u.ign||u.displayName||'Player')) + '</div></div>';
  ...
  window.findMatchByBudget = function() {
    ...
    for (var k in MT) {
      ...
      /* ✅ BUG FIX (2026-09-14): same class of bug as screens/home.js's
         mcHTML/renderHome — Number(t.entryFee)===0 fallback wrongly
         classified any match with a zero/corrupted entry_fee as Coin,
         even when entryType was genuinely 'paid'/Sky Diamond. That
         meant this affordability check compared the wrong currency
         balance (coins instead of money) for such matches. Category
         must come from entryType alone. */
  ...
  window.generateAdvancedPlayerCard = function() {
    ...
    h += '<div style="font-size:20px;font-weight:900;margin-bottom:2px">' + (window.escHtml?window.escHtml(UD.ign||'Player'):(UD.ign||'Player')) + '</div>';
  ...
  window.showWatchlistWithContext = function() {
    ...
    var h = '';
    if (UD && UD.ffUid) {
      h += '<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:rgba(0,255,156,.06);border-radius:8px;margin-bottom:10px;font-size:11px"><i class="fas fa-fingerprint" style="color:var(--green)"></i><span style="color:var(--txt2)">Watching as:</span><span style="font-weight:700;color:var(--green)">' + (window.escHtml?window.escHtml(UD.ign||'Player'):(UD.ign||'Player')) + ' · ' + (window.escHtml?window.escHtml(UD.ffUid||''):UD.ffUid||'') + '</span></div>';
    }
    if (!wl.length) {
  ...
  window.showQuickRules = window.showQuickRules || function() {
    var h = '<div>';
  ...
  window.showRecentWinners = window.showRecentWinners || function() {
    if (!db) return;
  ...
  window.coinToMatches = function() {
    ...
    for (var k in MT) {
      var t = MT[k];
      /* ✅ BUG FIX (2026-09-14): same class of bug as above — category
         must come from entryType, not from whether the fee happens to
         be zero (which can be a corrupted/unset paid-match fee, not a
         real Coin match). */
      var isCoin = (t.entryType||'').toLowerCase() === 'coin';
      if (!isCoin) continue;
      var fee = Number(t.entryFee) || 0;
  ...
  window.showPlayerOfTheDay = function() {
    ...
    db.ref('users').orderByChild('stats/wins').limitToLast(1).once('value', function(s) {
      ...
      h += '<div style="font-size:20px;font-weight:900">' + (window.escHtml?window.escHtml(top.ign||top.displayName||'Champion'):(top.ign||top.displayName||'Champion')) + '</div>';
  ...
  /* ✅ SECURITY FIX (2026-09-08): "server-authoritative wallet RPC"
     audit — this is the ACTUAL live cancelled-match-refund system
     (runs every 30s via setInterval below); screens/room.js's
     checkRefunds() implements the same feature but is never called
     from anywhere, so it was dead code. This function had the exact
     vulnerability the audit flagged: fee was read from jr.entryFee
     (this browser's own in-memory JR object, tamperable via devtools)
     and credited directly via a Firebase-bridge transaction() that
     ultimately called increment_balance with that client-supplied
     amount. Replaced with claim_match_refund(joinId) — the server
     re-derives the fee from the join_requests row's own
     entry_fee_paid column and re-verifies the match is genuinely
     cancelled; the client supplies only the join id. */
  ...
  window.confirmRewardRedeem = function(name, cost) {
    var U = window.U; var UD = window.UD; if (!window._supa||!U) return;
    var addr = (document.getElementById('rewardAddr')||{}).value||'';
    var phone = (document.getElementById('rewardPhone')||{}).value||'';
    /* ✅ SECURITY FIX (2026-09-08): "server-authoritative wallet RPC"
       audit — cost was a plain function argument, fully attacker-
       controlled if this function is called directly (e.g.
       window.confirmRewardRedeem('Gaming Headphones', 1)). Replaced
       with redeem_reward_item(name, address, phone) — the RPC itself
       looks up the real cost from a server-side items table
       (reward_store_items) and there is no cost parameter to tamper
       with at all. Client-side address/phone length checks are kept
       for instant feedback, but the RPC re-validates both server-side
       too. */
    ...
    window._supa.rpc('redeem_reward_item', { p_name: name, p_address: addr, p_phone: phone })
      .then(function(res) {
        var d = res && res.data;
        if (!d || !d.success) { _toast((d && d.error) || 'Redemption failed', 'err'); return; }
        if (window.UD) { window.UD.coins = d.newBalance; if (window.updateHdr) window.updateHdr(); }
        if (window.closeModal) closeModal();
        _toast('🎉 Reward redeemed! 7-10 din mein delivery hogi.', 'ok');
      }, function() { _toast('Redemption failed', 'err'); });
  };
