...
(function() {
  ...
  waitFor(function(){ return window.getUserClan !== undefined; }, function() {
    ...
    window.getUserClan = function(uid, cb) {
      ...
      window._supa.from('users').select('clan_id').eq('id', uid).maybeSingle()
        .then(function(r) {
          ...
          Promise.all([
            ...
            window._supa.from('users').select('id,ign,avatar_url,rank_points').in('id',
            ...
            window._supa.from('users').select('id,ign,avatar_url,rank_points')
              .in('id', memberIds)
              .then(function(ur) {
                var usersMap = {};
  ...
  waitFor(function(){ return window._supa !== undefined; }, function() {
    ...
    window._saveCreatorStats = function(uid, stats) {
      ...
      window._supa.from('creator_applications')
        ...
        .eq('user_id', uid)
        .catch(function(e) { console.warn('[Bug#3] Creator stats save failed:', e.message); });
    };
  ...
  waitFor(function(){ return window.startWatching !== undefined || window.startWatchEarn !== undefined; }, function() {
    ...
    window._getWatchEarnToday = function(uid, cb) {
      ...
      window._supa.from('watch_earn_log')
        ...
        .catch(function() { cb({ totalMins:0, totalCoins:0 }); });
    };
      ...
      window._supa.from('wallet_transactions').select('amount,txn_type,reason,created_at')
        ...
        .then(function(r) {
          ...
          var h = '<div style="padding:16px">' +
            '<div style="font-size:15px;font-weight:900;color:#ffd700;margin-bottom:12px">🪙 Coin History</div>';
          if (!rows.length) {
          ...
          h += '</div>';
          if (window.openModal) window.openModal(h, 'coin_history');
        })
        .catch(function(e) {
    ...
    window.updateBell = function() {
      ...
      window._supa.from('notifications')
        ...
        .then(function(r) {
          ...
          if (bell) {
            bell.textContent = count > 0 ? (count > 9 ? '9+' : String(count)) : '';
            bell.style.display = count > 0 ? '' : 'none';
          }
        })
        .catch(function() { if (_origUpdateBell) _origUpdateBell(); });
    };
      ...
      var _render = function() {
        ...
        _msgs.forEach(function(m) {
          ...
          h += '<div style="margin-bottom:6px;text-align:' + (isMe?'right':'left') + '">' +
            '<span style="font-size:10px;color:#666">' + (m.ign||'Player') + '</span><br>' +
      ...
      window._sendLobbyMsg = function(mId) {
        ...
        if (window._supa && uid) {
          window._supa.from('clan_messages').insert({
            clan_id: mId, user_id: uid, ign: ign, message: msg
          }).catch(function(){});
        }
        /* Optimistic local update */
        _msgs.push({ user_id: uid, ign: ign, message: msg, created_at: new Date().toISOString() });
        _render();
      };
    };
  ...
  waitFor(function(){ return window.deleteNotif !== undefined || window.delNotif !== undefined; }, function() {
    var _patchDeleteNotif = function(fnName) {
      ...
      window[fnName] = function(notifId) {
        ...
        if (window._supa && notifId) {
          window._supa.from('notifications').delete().eq('id', notifId).catch(function(){});
        }
      };
    };
