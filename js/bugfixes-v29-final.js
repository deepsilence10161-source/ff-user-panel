...
window.addEventListener('online', function() {
  ...
  queue.forEach(function(notifData) {
    window._supa.from('notifications').insert(notifData).catch(function(){});
  });
      ...
      if (path && path.startsWith('creatorStats/')) {
        ...
        return {
          ...
          set: function(value, cb) {
            if (!window._supa || !creatorUid) { if (cb) cb(null); return Promise.resolve(); }
            
            var updateData = {};
            if (statKey === 'totalEarnings' || statKey === 'total_earnings') {
              updateData.total_earnings = Number(value) || 0;
            } else if (statKey === 'referralCount' || statKey === 'referral_count') {
              updateData.referral_count = Number(value) || 0;
            } else if (statKey === 'activeReferrals') {
              updateData.active_referrals = Number(value) || 0;
            } else if (!statKey && typeof value === 'object') {
              /* Full stats update */
              if (value.totalEarnings !== undefined) updateData.total_earnings = Number(value.totalEarnings) || 0;
              if (value.referralCount !== undefined) updateData.referral_count = Number(value.referralCount) || 0;
              if (value.activeReferrals !== undefined) updateData.active_referrals = Number(value.activeReferrals) || 0;
              if (value.creatorCode) updateData.creator_code = value.creatorCode;
              if (value.status) updateData.status = value.status;
            }
            
            if (Object.keys(updateData).length) {
              return window._supa.from('creator_applications')
                .upsert({ user_id: creatorUid, ...updateData }, { onConflict: 'user_id' })
          ...
          update: function(value, cb) { return this.set(value, cb); },
          once: function(ev, cb) {
            if (!window._supa) { if (cb) cb({ val: function(){ return null; }, exists: function(){ return false; } }); return; }
            window._supa.from('creator_applications').select('*').eq('user_id', creatorUid).maybeSingle()
              .then(function(r) {
                if (cb) cb({ val: function(){ return r.data; }, exists: function(){ return !!r.data; }, key: creatorUid });
              }).catch(function() {
                if (cb) cb({ val: function(){ return null; }, exists: function(){ return false; } });
              });
          },
          on: function(ev, cb) { this.once(ev, cb); return this; },
          off: function() {},
          transaction: function(fn, cb) { if (cb) cb(null, false, null); },
          push: function(v, cb) { if (cb) cb(null); return this; }
        };
      }
      return _origRef(path);
    };
    console.log('[Fix M-4] creatorStats full data write patched ✅');
  }, 400);
}
