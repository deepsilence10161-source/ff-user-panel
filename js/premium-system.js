...
window.getUserPremiumTier=function(){
  ...
  if(p.expiresAt&&Date.now()>p.expiresAt) return 0;
  return p.tier||0;
};
...
window.showWithdrawalPolicy=function(onAccept){
  ...
  window._confP=function(){
    if(!_acc)return;
    /* ✅ Bug 11 Fix: Store in BOTH localStorage AND Supabase */
    if(window.U){
      localStorage.setItem('_mes_policy_'+window.U.uid,'1');
      /* Server-side persistence — survives browser data clear */
      if(window._supa){
    ...
    setTimeout(function(){if(onAccept)onAccept();},250);
};
  if(window.openModal)openModal('📋 Withdrawal Policy',h);
};
window.checkPolicyThenRun=function(fn){
  if(!window.U)return;
  /* Check localStorage first (fast path) */
  if(localStorage.getItem('_mes_policy_'+window.U.uid)==='1'){fn();return;}
  /* ✅ Bug 11 Fix: Also check Supabase (survives localStorage clear) */
  if(window._supa && window.UD){
    if(window.UD.accepted_policy === true){
      localStorage.setItem('_mes_policy_'+window.U.uid,'1'); /* Re-cache */
      fn(); return;
    }
    /* Live server check */
    window._supa.from('users').select('accepted_policy').eq('id',window.U.uid).single()
      .then(function(r){
        if(r.data && r.data.accepted_policy === true){
          localStorage.setItem('_mes_policy_'+window.U.uid,'1');
          fn();
        } else {
          window.showWithdrawalPolicy(fn);
        }
      }).catch(function(){ window.showWithdrawalPolicy(fn); });
  } else {
    window.showWithdrawalPolicy(fn);
  }
};
...
function _renderPremModal(){
  ...
  TIERS.forEach(function(t){
    ...
    h+='<div style="background:'+t.bg+';border:'+(isA?'2px':'1.5px')+' solid '+(isA?t.color:t.border)+';border-radius:18px;padding:'+(iBest?'20px 16px 16px':'16px')+';box-shadow:'+(iBest?'0 0 24px '+t.glow+',0 6px 28px rgba(0,0,0,.5)':'0 0 12px rgba(0,0,0,.3)')+';">';
...
window.buyPremium=function(tier,price){
  ...
  window._submitPremium=function(tier,price){if(!_ss){if(window.toast)toast('Screenshot upload karo!','err');return;}if(!window.U||!window.UD||!window.db)return;var id=window.db.ref('premiumRequests').push().key;window.db.ref('premiumRequests/'+id).set({uid:window.U.uid,userName:window.UD.ign||window.UD.displayName||'',email:window.UD.email||'',tier:tier,price:price,screenshotBase64:_ss,status:'pending',createdAt:Date.now()});window.db.ref('users/'+window.U.uid+'/notifications').push({title:'💳 Premium Request Received!',message:'Premium '+TIERS[tier-1].label+' (₹'+price+') request mila! 1-2 ghante mein activate hoga. '+TIERS[tier-1].gdBonus+' Green Diamonds bhi milenge.',timestamp:Date.now(),createdAt:Date.now(),read:false,type:'premium'});if(window.toast)toast('✅ Request submit! 1-2 ghante mein active.','ok');if(window.closeModal)closeModal();};
};
