...
(function() {
  ...
  window._reqNotifPerm = function() {
    /* Bug #47 Fix: Check current permission state before requesting
       iOS Safari requires user gesture AND only fires once per session */
    if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
      console.log('[Notif] Already granted');
      return;
    }
    if (typeof Notification !== 'undefined' && Notification.permission === 'denied') {
      if (window.toast) toast('Notifications blocked hain — browser settings mein enable karo', 'err');
      return;
    }
