...
(function () {
  ...
  window.mesConfirmAge = function () {
    ...
    if (window._supa && window.U && window.U.uid) {
      window._supa.from('users').update({
        age_verified: true, date_of_birth: dob, age_verified_at: new Date().toISOString()
      }).eq('id', window.U.uid).then(function(res) {
        if (res && res.error) {
  ...
  window.mesAcceptTC = function (uid) {
    ...
    if (window.DB && window.DB.users) {
      window.DB.users.update({ accepted_policy: true }).catch(function(){});
    }
    ...
    window.db.ref('appSettings/tdsConfig').once('value', function(cfgSnap) {
      ...
      window.db.ref('payoutLogs').orderByChild('timestamp').startAt(thirtyDaysAgo).once('value', function(s) {
        ...
        window.db.ref('users').once('value', function(us) {
          ...
          if (window._supa) {
            window._supa.from('admin_activity_log').insert({action_type:'TDS_AUTO_ACTIVATED',details:{triggered:triggered,financialYear:fy},status:'open'}).catch(function(){});
          }
