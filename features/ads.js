...
/* Issue #28 Fix: Cache premium status for 30s — _adIsPremium() was called on
   every ad event causing unnecessary Firebase reads on each banner render. */
var _premCache = { tier: 0, exp: 0, checked: 0 };
function _adIsPremium() {
  var now = Date.now();
  if (now - _premCache.checked < 30000) {
    /* Serve from cache */
...
window.watchAdForCoins = function() {
  ...
  window.AdManager.showRewardedAd(
    function() {
      ...
      localStorage.setItem(_adStorKey, String(watchedToday + 1));
