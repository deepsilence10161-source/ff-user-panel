...
window.checkStreakMilestones = function() {
  ...
  MILESTONES.forEach(function(m) {
    if (streak >= m.day && !claimed['day_' + m.day]) {
      /* BUG FIX (2026-07-30): previously read the local UD cache to decide whether this
         milestone was already claimed, then unconditionally wrote coins — if UD was
         stale (hadn't yet picked up a previous claim), this could re-credit the same
         milestone's coins more than once (this was flagged as a known risk in an
         earlier audit pass — "stale local UD cache" — but not actually fixed until now).
         Fixed: the claimed-flag write is now the atomic gate via a Firebase transaction.
         Coins are only credited in the completion callback, and only if this specific
         call is the one that actually flipped false→true (checked via committed +
         snapshot value, not just "didn't error") — so even if checkStreakMilestones()
         runs twice concurrently (e.g. called from the profile listener AND from the
         post-check-in wrapper within the same second), only one of them can win the
         transaction and credit the coins. */
      db().ref('users/' + uid() + '/streakMilestonesClaimed/day_' + m.day)
        .transaction(function(current) {
          if (current === true) return; /* already claimed — abort this transaction */
          return true;
        }, function(err, committed, snap) {
          if (err || !committed || !snap || snap.val() !== true) return; /* lost the race or already claimed */
...
window.checkPremiumMonthlyBonus = function() {
  ...
  db().ref('users/' + uid() + '/premiumMonthlyBonus/' + monthKey).once('value', function(s) {
    ...
    /* ✅ Sync to Supabase */
    if (window._supa && uid()) {
      window._supa.rpc('increment_balance', { p_uid: uid(), p_col: 'coins', p_amount: bonus }).then(null, function(){});
      window._supa.from('wallet_transactions').insert({ user_id: uid(), currency: 'coins', txn_type: 'credit', amount: bonus, reason: 'premium_bonus', note: 'Monthly Premium Bonus Tier ' + tier }).then(null, function(){});
    }
...
window.submitCreatorSignup = function() {
  ...
  window._supa.from('creator_applications').insert({
    ...
    if (r.error) {
      var msg = r.error.code === '23505' ? 'Ye code pehle se liya hua hai — doosra code try karo' : (r.error.message || 'Submit failed');
      toast(msg, 'err');
      return;
    }
