...
window.doShareCreatorLink = function(encodedMsg) {
  /* BUG FIX (2026-07): native whatsapp:// scheme, opens app directly */
  window.open('whatsapp://send?text=' + encodeURIComponent(decodeURIComponent(encodedMsg)), '_self');
};
...
window.requestCreatorPayout = function(amount) {
  ...
  /* ✅ BUG FIX (2026-07-19, CRITICAL): was Firebase-only (a transaction
     resetting the RTDB pendingPayout to 0) — creator_stats never had this
     mirrored at all, both because of a field-name mismatch in the bridge
     handler and because creator_stats had no INSERT/UPDATE RLS policy of
     any kind (see the creator_stats table comment in COMPLETE_SCHEMA.sql
     for the full history). claim_creator_payout does the real Supabase
     side: atomically reads pending_payout, zeroes it, and adds it to
     paid_out — one authoritative source of truth for what's actually
     been paid instead of two, silently diverging, copies of the number. */
  if (window._supa) {
    window._supa.rpc('claim_creator_payout').then(function(res) {
      if (res.error || (res.data && res.data.success === false)) {
        var msg = (res.data && res.data.error) || (res.error && res.error.message) || 'Unknown error';
        console.error('[Creator] claim_creator_payout failed:', msg);
      }
    });
  }
...
window.trackCreatorSale = function(amount, creatorCode) {
  ...
  db().ref('creatorCodes/' + creatorCode.toUpperCase()).once('value', function(s) {
    ...
    /* ✅ BUG FIX (2026-07-19, CRITICAL): the two transactions above were
       Firebase-only — creator_stats never actually received this lock in
       Supabase (field-name mismatch in the bridge handler AND missing
       RLS policies, see the creator_stats table comment in
       COMPLETE_SCHEMA.sql). lock_creator_commission gives this a real
       Supabase-side counterpart. */
    if (window._supa) {
      window._supa.rpc('lock_creator_commission', { p_creator_uid: creatorUid, p_amount: commission })
        .then(function(res) {
          if (res.error || (res.data && res.data.success === false)) {
            console.error('[Creator] lock_creator_commission failed:', (res.data && res.data.error) || (res.error && res.error.message));
          }
        });
    }
...
window.releaseCreatorCommissionIfPending = function(uid) {
  ...
  db().ref('users/' + uid + '/_pendingCreatorCommission').once('value', function(s) {
    ...
    /* ✅ BUG FIX (2026-07-19, CRITICAL): the three transactions above were
       Firebase-only — the actual lock→release money-movement in
       creator_stats never reached Supabase (same root cause as
       trackCreatorSale/requestCreatorPayout above). release_creator_commission
       does the real, atomic locked_commission → total_commission +
       pending_payout movement server-side. */
    if (window._supa) {
      window._supa.rpc('release_creator_commission', { p_creator_uid: pending.creatorUid, p_amount: pending.amount })
        .then(function(res) {
          if (res.error || (res.data && res.data.success === false)) {
            console.error('[Creator] release_creator_commission failed:', (res.data && res.data.error) || (res.error && res.error.message));
          }
        });
    }
