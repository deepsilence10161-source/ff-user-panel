/* ================================================================
   ...
   Tables: friendships(user1_id,user2_id), user_activities(uid,ign,type,text), notifications
================================================================ */
(function(){ 'use strict';
...
function _frList(c){
  ...
  _s().from('friendships').select('user1_id,user2_id').or('user1_id.eq.'+_uid()+',user2_id.eq.'+_uid())
  .then(function(r){
    var ids=(r.data||[]).map(function(f){return f.user1_id===_uid()?f.user2_id:f.user1_id;});
    ...
    _s().from('users').select('id,ign,avatar_url,rank_points,total_matches,total_wins').in('id',ids)
    .then(function(u){
      var list=u.data||[];
...
function _frActivity(c){
  ...
  _s().from('friendships').select('user1_id,user2_id').or('user1_id.eq.'+_uid()+',user2_id.eq.'+_uid())
  .then(function(r){
    var ids=[_uid()].concat((r.data||[]).map(function(f){return f.user1_id===_uid()?f.user2_id:f.user1_id;}));
    _s().from('user_activities').select('*').in('uid',ids).order('created_at',{ascending:false}).limit(30)
    .then(function(ar){
      var acts=ar.data||[];
...
window.frSearchUser=function(q){
  ...
  _frTimer=setTimeout(function(){
    ...
    _s().from('users').select('id,ign,ff_uid,avatar_url,rank_points,total_matches').or('ign.ilike.%'+q+'%,ff_uid.ilike.%'+q+'%').eq('is_banned',false).neq('id',_uid()).limit(10)
    .then(function(res){
      ...
      .then(function(fr){
        if(_myReqId !== _frSearchReqId) return; /* ✅ Stale response — discard */
        var myFriends={};(fr.data||[]).forEach(function(f){var fid=f.user1_id===_uid()?f.user2_id:f.user1_id;myFriends[fid]=true;});
        var h='<div style="display:flex;flex-direction:column;gap:7px">';
...
window.addFriend=function(uid,ign,btn){
  ...
  _s().from('friendships').upsert([{user1_id:_uid(),user2_id:uid},{user1_id:uid,user2_id:_uid()}],{onConflict:'user1_id,user2_id',ignoreDuplicates:true})
  .then(function(){
...
window.removeFriend=function(uid){
  ...
  _s().from('friendships').delete().or('and(user1_id.eq.'+_uid()+',user2_id.eq.'+uid+'),and(user1_id.eq.'+uid+',user2_id.eq.'+_uid()+')')
...
window.logActivity=function(type,text){
  ...
  _s().from('user_activities').insert({uid:_uid(),ign:_ud().ign||_ud().displayName||'Player',type:type,text:text})
  .catch(function(){});
};
