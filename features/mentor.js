...
function _ri(p){ p=Number(p)||0;if(p>=1501)return{b:'Diamond',e:'💎',c:'#00d4ff',bg:'rgba(0,212,255,.12)'};if(p>=601)return{b:'Gold',e:'🥇',c:'#ffd700',bg:'rgba(255,215,0,.12)'};if(p>=301)return{b:'Silver',e:'🥈',c:'#c0c0c0',bg:'rgba(192,192,192,.10)'};return{b:'Bronze',e:'🏅',c:'#cd7f32',bg:'rgba(205,127,50,.12)'}; }
function _tier(p){ p=Number(p)||0;if(p>=1501)return 4;if(p>=601)return 3;if(p>=301)return 2;return 1; }
...
function _mtFind(c){
  ...
  _s().from('mentor_profiles').select('*').eq('active',true).order('rank_pts',{ascending:false}).limit(20)
  .then(function(r){
    var list=(r.data||[]).filter(function(m){return m.uid!==_uid();});
    ...
    list.forEach(function(m){
      ...
      if(m.bio)h+='<div style="font-size:12px;color:var(--txt2);font-style:italic;margin-bottom:10px">"'+m.bio.substring(0,80)+'"</div>';
...
function _mtMySessions(c){
  ...
  _s().from('mentor_requests').select('*').eq('student_id',_uid()).order('created_at',{ascending:false}).limit(10)
  .then(function(r){
    var reqs=r.data||[];
...
function _mtDashboard(c){
  ...
  c.innerHTML='<div style="text-align:center;padding:16px;color:var(--txt2)"><i class="fas fa-spinner fa-spin"></i></div>';
  if(!_s()||!_uid()){c.innerHTML='<p style="color:#ff6b6b;text-align:center">Login required</p>';return;}
  _s().from('mentor_profiles').select('*').eq('uid',_uid()).single()
  .then(function(r){
    if(!r.data||!r.data.active)_renderBecomeMentor(c);
...
function _renderMentorStats(c,m){
  ...
  c.innerHTML=h;
  _s().from('mentor_requests').select('*').eq('mentor_id',_uid()).eq('status','pending').order('created_at',{ascending:false}).limit(10)
  .then(function(r){
...
window.registerAsMentor=function(){
  ...
  _s().from('mentor_profiles').upsert({uid:_uid(),ign:ud.ign||ud.displayName||'Player',rank_pts:ud.rank_points||0,rank_tier:ri.b,avatar_url:ud.avatar_url||'',speciality:spec.trim(),bio:bio.trim(),active:true,total_sessions:0,total_students:0,successful_students:0,gd_earned:0},{onConflict:'uid'})
...
window._submitMentorRequest=function(mentorUid,mentorIgn){
  ...
  var ud=_ud();var ri=_ri(ud.rank_points||0);
  _s().from('mentor_requests').insert({mentor_id:mentorUid,mentor_ign:mentorIgn,student_id:_uid(),student_ign:ud.ign||ud.displayName||'Player',student_rank_pts:ud.rank_points||0,student_rank_badge:ri.b,message:msg.trim(),status:'pending'})
  .then(function(){
...
window.acceptMentorReq=function(reqId){
  if(!_s()||!_uid())return;
  _s().from('mentor_requests').select('student_id,student_ign').eq('id',reqId).single()
  .then(function(r){
    var req=r.data;if(!req)return;
    _s().from('mentor_requests').update({status:'accepted'}).eq('id',reqId).then(function(){
      _s().from('notifications').insert({user_id:req.student_id,type:'mentor_accepted',title:'🎉 Mentor Accepted!',body:(_ud().ign||'Mentor')+' ne tumhari request accept kar li!',ref_id:_uid(),is_read:false}).catch(function(){});
      _s().from('mentor_profiles').select('total_students').eq('uid',_uid()).single().then(function(m){
        if(m.data)_s().from('mentor_profiles').update({total_students:(m.data.total_students||0)+1}).eq('uid',_uid()).catch(function(){});
      }).catch(function(){});
...
window.deactivateMentor=function(){
  if(!_s()||!_uid())return;
  _s().from('mentor_profiles').update({active:false}).eq('uid',_uid()).then(function(){if(window.toast)toast('Mentor profile paused','ok');if(window.closeModal)closeModal();}).catch(function(){});
};

window.checkMentorReward=function(oldRp,newRp){
  if(!_s()||!_uid())return;
  var oldT=_tier(oldRp),newT=_tier(newRp);if(newT<=oldT)return;
  _s().from('mentor_requests').select('mentor_id,mentor_ign').eq('student_id',_uid()).eq('status','accepted').limit(1).single()
  .then(function(r){
    if(!r.data)return;
    var mid=r.data.mentor_id,mign=r.data.mentor_ign;
    var gd=20*(newT-oldT);
    _s().from('users').select('green_diamonds').eq('id',mid).single().then(function(u){
      ...
      _s().from('mentor_profiles').select('gd_earned,successful_students').eq('uid',mid).single().then(function(mp){
        if(!mp.data)return;
        _s().from('mentor_profiles').update({gd_earned:(mp.data.gd_earned||0)+gd,successful_students:(mp.data.successful_students||0)+1}).eq('uid',mid).catch(function(){});
      }).catch(function(){});
