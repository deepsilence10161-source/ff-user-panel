...
function giveShareCoins() {
  ...
  window.db.ref(key).once('value', function(s) {
    ...
    /* BUG #35 FIX (2026-07): this used to write coins via BOTH the bridge transaction
       (which maps straight to the same `coins` column) AND the increment_balance RPC —
       a straightforward double-credit on every share. Now only the RPC call remains. */
    if (window._supa && window.U) { window._supa.rpc('increment_balance', { p_uid: window.U.uid, p_col: 'coins', p_amount: _shareCoins }).then(null, function(){}); }
    if (window.UD) { window.UD.coins = (window.UD.coins||0) + _shareCoins; if (window.updateHdr) updateHdr(); }
    window.db.ref(key).set(true);
    /* ✅ FIX (2026-08-17): giveShareCoins() and the Weekly Mission
       "Result Share Karo" card were two completely disconnected reward
       systems — sharing here paid its own +20 coins but never set
       wShare_{weekNum}, which is the ONLY thing the Missions panel's
       week_share card checks. That meant the Weekly Mission card could
       never show complete/claimed, and its separate +20 reward was
       permanently unreachable ("result share karne par koi coin nahi").
       Now this also marks mission progress, so the two stay in sync. */
    var _weekNum = (typeof getWeekNum === 'function') ? getWeekNum() : null;
    if (_weekNum !== null) {
      window.db.ref('users/' + window.U.uid + '/missionProgress/wShare_' + _weekNum).set(true);
    }
    toast('+' + _shareCoins + ' 🪙 Coins mile! Share karte raho!', 'ok');
