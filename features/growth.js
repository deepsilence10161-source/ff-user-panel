...
window.doShareResult = function(encodedMsg) {
  ...
  /* BUG FIX (2026-07): '_blank' makes the Android WebView open a popup
     window (onCreateWindow in MainActivity.java) which tries to render
     wa.me as a normal webpage instead of handing off to the WhatsApp app.
     '_self' navigates in the main WebView, which DOES correctly hand
     https:// links to Android via ACTION_VIEW so WhatsApp opens properly. */
...
window.doShareReferral = function(encodedMsg) {
  var msg = decodeURIComponent(encodedMsg);
  /* BUG FIX (2026-07): see doShareResult above — '_self' instead of
     '_blank' so this actually opens WhatsApp instead of silently failing
     inside the Android WebView's popup window. */
