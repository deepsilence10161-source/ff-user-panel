...
window._doContribute=function(clanId){
  ...
  // Deduct from user
  /* Bug H-5 Fix: Use atomic decrement_balance RPC to prevent race condition */
  /* Old code: _s().from('users').update({green_diamonds:myGd-amt}) — NOT atomic! */
  _s().rpc('decrement_balance', { p_uid: _uid(), p_col: 'green_diamonds', p_amount: amt })
  .then(function(){
