/* ================================================================
   ...
   Tables: clan_wars(id,week,clan1_id,clan1_name,clan2_id,clan2_name,clan1_score,clan2_score,status,winner_id)
...
function _loadClanWarData(myClanId){
  ...
  var week=_week();
  _s().from('clan_wars').select('*').eq('week',week).or('clan1_id.eq.'+myClanId+',clan2_id.eq.'+myClanId).eq('status','active').single()
...
function _renderActiveWar(mb,myClanId,war,week){
  ...
  var isClan1=war.clan1_id===myClanId;
  var myClanName=isClan1?war.clan1_name:war.clan2_name;
  var oppName=isClan1?war.clan2_name:war.clan1_name;
  var myScore=isClan1?(war.clan1_score||0):(war.clan2_score||0);
  var oppScore=isClan1?(war.clan2_score||0):(war.clan1_score||0);
...
function _renderWarLobby(mb,myClanId,week){
  ...
  mb.innerHTML=h;
  _s().from('clan_wars').select('clan1_id,clan1_name,clan2_id,clan2_name,clan1_score,clan2_score').eq('week',week)
  .then(function(r){
    var wars=r.data||[]; var cityScores={};
    wars.forEach(function(w){
      if(!cityScores[w.clan1_id])cityScores[w.clan1_id]={name:w.clan1_name,score:0};
      if(!cityScores[w.clan2_id])cityScores[w.clan2_id]={name:w.clan2_name,score:0};
      cityScores[w.clan1_id].score+=(w.clan1_score||0);
      cityScores[w.clan2_id].score+=(w.clan2_score||0);
    });
...
window._sendWarChallenge=function(fromId,toId,toName,fromName,btn){
  ...
  var week=_week();
  _s().from('clan_war_challenges').insert({week:week,from_clan:fromId,from_name:fromName,to_clan:toId,to_name:toName,status:'pending'})
  .then(function(){
    // Notify clan leader
    _s().from('clans').select('leader_id').eq('id',toId).single().then(function(r){
      if(r.data&&r.data.leader_id){
        _s().from('notifications').insert({user_id:r.data.leader_id,type:'clan_war_challenge',title:'⚔️ Clan War Challenge!',body:fromName+' ne tumhare clan ko war challenge bheja!',ref_id:null,is_read:false}).catch(function(){});
      }
    }).catch(function(){});
...
window.updateClanWarScore=function(wins,kills){
  ...
  _s().from('clan_wars').select('id,clan1_id,clan1_score,clan2_score').eq('week',week).or('clan1_id.eq.'+clanId+',clan2_id.eq.'+clanId).eq('status','active').single()
  .then(function(r){
    if(!r.data)return;
    var isClan1=r.data.clan1_id===clanId;
    var col=isClan1?'clan1_score':'clan2_score';
