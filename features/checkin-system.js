    ...
    .then(function(r) {
      ...
      noShows.forEach(function(jr) {
        ...
        if (fee > 0 && etype !== 'free' && etype !== 'ad') {
          ...
          window._supa.rpc('increment_balance', { p_uid: jr.user_id, p_col: col, p_amount: fee }).then(null, function(){});
          window._supa.from('wallet_transactions').insert({ user_id: jr.user_id, currency: col, txn_type: 'credit', amount: fee, reason: 'no_show_refund', ref_id: matchId }).then(null, function(){});
        }
