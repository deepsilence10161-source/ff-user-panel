...
window.sharePlayerCardGeneric=function(){
  ...
  if(navigator.share){ navigator.share({title:'My Mini eSports Card',text:msg}).catch(function(){}); }
  else if(navigator.clipboard){ navigator.clipboard.writeText(msg); toast('Card text copy ho gaya!','ok'); }
};
...
window.showAutoResultCard = function(matchName, rankPos, kills, matchId){
  ...
  if(isWin && window._supa && window.U){
    var coins=rankPos===1?50:rankPos===2?30:rankPos===3?20:5;
    var curCoins=(window.UD&&window.UD.coins)||0;
    window._supa.from('users').update({coins:curCoins+coins}).eq('id',window.U.uid)
    .then(function(){
      if(window.UD) window.UD.coins=curCoins+coins;
