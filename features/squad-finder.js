...
window.sfSubmitPost=function(){
  ...
  _supa().from('squad_finder').upsert({ user_id:_uid(),ign:ud.ign||ud.displayName||'Player',rank_pts:rp,rank_tier:_tier(rp),mode:modeSel?modeSel.dataset.sel:'Any',role:roleSel?roleSel.dataset.sel:'',lang:langSel?langSel.dataset.sel:'',note:note.trim(),is_active:true,expires_at:expires },{onConflict:'user_id'})
