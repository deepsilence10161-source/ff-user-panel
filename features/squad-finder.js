...
function _tier(p){ p=Number(p)||0; if(p>=1501)return'Diamond'; if(p>=601)return'Gold'; if(p>=301)return'Silver'; return'Bronze'; }
function _rankInfo(p){ var t=_tier(p); return {Diamond:{e:'💎',c:'#00d4ff',bg:'rgba(0,212,255,.12)'},Gold:{e:'🥇',c:'#ffd700',bg:'rgba(255,215,0,.12)'},Silver:{e:'🥈',c:'#c0c0c0',bg:'rgba(192,192,192,.10)'},Bronze:{e:'🏅',c:'#cd7f32',bg:'rgba(205,127,50,.12)'}}[t]||{e:'🏅',c:'#cd7f32',bg:'rgba(205,127,50,.12)'}; }
...
window._sfRefreshBrowse=function(){
  ...
  var q=_supa().from('squad_finder').select('*').eq('active',true).gt('expires_at',new Date().toISOString()).order('created_at',{ascending:false}).limit(50);
  ...
  q.then(function(r){
    ...
    list.forEach(function(d){
      ...
      h+='<button onclick="window.sfSendRequest(\''+d.uid+'\',\''+(d.ign||'Player').replace(/'/g,"\\'")+'\',this)" style="padding:8px 12px;border-radius:10px;border:1.5px solid rgba(0,255,156,.4);background:rgba(0,255,156,.08);color:var(--green);font-size:11px;font-weight:800;cursor:pointer;flex-shrink:0">Invite</button>';
      h+='</div>';
    });
...
window.sfSubmitPost=function(){
  ...
  var ud=_ud(),rp=ud.rank_points||0;
  var expires=new Date(Date.now()+10800000).toISOString();
  _supa().from('squad_finder').upsert({ uid:_uid(),ign:ud.ign||ud.displayName||'Player',rank_pts:rp,rank_tier:_tier(rp),mode:modeSel?modeSel.dataset.sel:'Any',role:roleSel?roleSel.dataset.sel:'',lang:langSel?langSel.dataset.sel:'',note:note.trim(),active:true,expires_at:expires },{onConflict:'uid'})
  .then(function(){ _toast('✅ LFS Post ho gayi!','ok'); if(window.closeModal)closeModal(); })
