...
window.startWatching = function(matchId) {
  ...
  window.db.ref('users/' + window.U.uid + '/watchEarnings/' + today).once('value', function(snap) {
    ...
    // Update spectator count — Firebase (real-time) + Supabase (Issue #15 Fix)
    window.db.ref('matches/' + matchId + '/spectatorCount').transaction(function(v) {
      return (v||0) + 1;
    });
    if (window._supa) {
      window._supa.from('matches').select('spectator_count').eq('firebase_id', matchId).single()
        .then(function(r) {
          var cur = (r.data && r.data.spectator_count) || 0;
          window._supa.from('matches').update({ spectator_count: cur + 1 })
            .eq('firebase_id', matchId).catch(function(){});
        }).catch(function(){});
    }
    ...
    _watchInterval = setInterval(function() {
      ...
      /* Bug 13 Fix: Check match is still live before awarding coins.
         Without this check coins keep flowing even after match ends/is cancelled. */
      var mt = window.MT && window.MT[_watchMatchId];
      var matchStatus = mt && (mt.status || mt.matchStatus || '');
      if (matchStatus === 'completed' || matchStatus === 'cancelled' || matchStatus === 'resultPublished' || (mt && mt.resultPublished)) {
        clearInterval(_watchInterval);
        _watchMatchId = null;
        toast('Match khatam ho gaya — Watch & Earn ended.', 'inf');
        var watchUI = document.getElementById('watchEarnUI');
        if (watchUI) watchUI.remove();
        return;
      }
      /* Also check Firebase match status for accuracy */
      window.db.ref('matches/' + _watchMatchId + '/status').once('value', function(s) {
        var st = s.val() || '';
        if (st === 'completed' || st === 'cancelled' || st === 'resultPublished') {
          clearInterval(_watchInterval);
          _watchMatchId = null;
          toast('Match khatam ho gaya — Watch & Earn ended.', 'inf');
          return;
...
/* ── Bug 9 Fix: Auto-stop watching when user leaves tab / closes app ── */
(function() {
  function _autoStopIfNeeded() {
    if (window._watchMatchId && typeof window.stopWatching === 'function') {
      console.log('[WatchEarn] Tab hidden/unloaded — auto stopping watch session');
      window.stopWatching();
    }
  }
  /* visibilitychange: tab background mein jaaye tab stop */
  document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'hidden') _autoStopIfNeeded();
  });
  /* pagehide: app close ya navigate away */
  window.addEventListener('pagehide', _autoStopIfNeeded);
  /* beforeunload: browser close */
  window.addEventListener('beforeunload', _autoStopIfNeeded);
})();
