...
window.startFreeTrial = function() {
  ...
  if (window.isTrialUsed()) { if(window.toast)toast('Trial pehle use ho chuka hai','inf'); return; }
  if (window.getUserPremiumTier && window.getUserPremiumTier() > 0) {
    if(window.toast)toast('Aap already premium mein hain!','inf'); return;
  }
  ...
  /* Apply trial perks immediately (same as Silver) */
  _applyTrialPerks();

  /* Log to Supabase for tracking */
  if (window._supa) {
    window._supa.from('trial_log').insert({
      user_id: uid,
