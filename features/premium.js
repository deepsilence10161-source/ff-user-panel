...
/* ✅ CUSTOMIZABLE (2026-09-09), per Junaid's request: "har chiz jo
   ...
   (app_settings.live_config.premium — the same key App Settings'
   "Premium Subscription" card already saves to). Built as a function
   (called fresh each time the premium modal opens) rather than a
   module-load-time constant, since window.CFG.premium is only
   populated by an async fetch (loadAppConfig()) that may not have
   resolved yet when this script file first runs — building TIERS
   fresh on each modal open guarantees it always reflects whatever
   config has loaded by the time the user actually opens the screen.
   Perks/labels/colors stay hardcoded since those are feature copy,
   not a number an admin would tune — Junaid's ask was specifically
   about prices/limits. */
function _buildTiers(){
  var _premCfg = (window.CFG && window.CFG.premium) || {};
...
window.getUserPremiumTier=function(){
  ...
  var p=ud.premium;
  /* BUG #4/#5/#29 FIX (2026-07): there is no separate "active" boolean anywhere in the
     Supabase schema (users has premium_level + premium_expires only) and nothing ever
     populated ud.premium.active — this always evaluated to 0 for every user, even ones
     admin had approved. Active status is now correctly derived from tier+expiry directly. */
  if(!p.tier||p.tier<=0) return 0;
...
function _renderPremModal(){
  ...
  var TIERS=_buildTiers(); // fresh read of window.CFG.premium every time the modal opens
  window.PREMIUM_TIERS=TIERS;
