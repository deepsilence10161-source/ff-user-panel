...
window.awardPassXP=function(uid,xp){
  ...
  _bpXPTimers[uid] = setTimeout(function() {
    ...
    window._supa.rpc('award_battle_pass_xp', { p_uid: uid, p_season: sid, p_xp: totalXP })
      .then(function() { console.log('[BattlePass] XP awarded via RPC:', totalXP); })
      .catch(function() {
...
/* ── Claim reward (Bug 65 Fix: Supabase instead of RTDB) ── */
/* Bug #14 Fix: Server-side tier validation + in-progress guard */
...
window.claimPassReward=function(tierNum,track){
  ...
  _getBPProgress(uid, function(d) {
    if (!d) { _bpClaimInProgress[claimKey]=false; return; }
    var claimed = track==='free' ? (d.claimed_free||{}) : (d.claimed_prem||{});
    if (claimed[String(tierNum)]) { _bpClaimInProgress[claimKey]=false; if(window.toast)toast('Yeh already claim ho chuka!','inf'); return; }
    /* Bug #14: SERVER VALIDATION — verify user actually has this tier from DB */
    if ((d.current_tier||0) < tierNum) {
      _bpClaimInProgress[claimKey]=false;
      if(window.toast)toast('Is tier ke liye pehle XP earn karo!','err');
      return;
    }
    /* Bug #14: Premium track validation */
    if (track==='prem' && !d.has_premium) {
      _bpClaimInProgress[claimKey]=false;
      if(window.toast)toast('Premium pass chahiye is reward ke liye!','err');
      return;
    }

    /* Mark claimed */
    claimed[String(tierNum)] = true;
    var update = track==='free'
      ? { claimed_free: claimed, updated_at: new Date().toISOString() }
      : { claimed_prem: claimed, updated_at: new Date().toISOString() };

    window._supa.from('battle_pass_progress')
      .update(update)
      .eq('user_id', uid).eq('season_key', sid)
      .then(function() {
        /* Award GD if applicable */
        var gdAmt = (reward.type==='gd' ? reward.v : 0) + (reward.extraGD || 0);
        if (gdAmt > 0) {
          window._supa.rpc('increment_balance',{p_uid:uid,p_col:'green_diamonds',p_amount:gdAmt}).catch(function(){});
          window._supa.from('wallet_transactions').insert({
            user_id: uid, currency: 'green_diamonds', txn_type: 'credit',
            amount: gdAmt, reason: 'battle_pass_tier_'+tierNum+'_'+track
          }).catch(function(){});
