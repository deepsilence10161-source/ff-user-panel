  ...
  window.db.ref(claimKey).once('value',function(s){
    ...
    if(reward.type==='gd'||reward.extraGD){
      ...
      if(gdAmt>0){
        ...
        /* ✅ Sync green diamonds to Supabase */
        if(window._supa){window._supa.rpc('increment_balance',{p_uid:uid,p_col:'green_diamonds',p_amount:gdAmt}).catch(function(){});window._supa.from('wallet_transactions').insert({user_id:uid,currency:'green_diamonds',txn_type:'credit',amount:gdAmt,reason:'battle_pass_reward'}).catch(function(){});}
      }
    }
