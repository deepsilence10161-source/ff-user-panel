...
function doGoogleLogin() {
  ...
  auth.signInWithPopup(gp).then(function() {
    ...
    if (code === 'auth/popup-closed-by-user') {
      ...
      toast('Redirect se login try kar raha hoon...', 'inf');
      setTimeout(function() { auth.signInWithRedirect(gp).catch(function(e2){ toast(e2.message||'Login failed','err'); }); }, 500);
    } else if (code === 'auth/network-request-failed') {
      toast('Network error — internet check karo', 'err');
    } else if (code === 'auth/unauthorized-domain') {
      var lh = document.querySelector('.login-help');
      if (lh) lh.innerHTML = '<span style="color:#ff6b6b;font-weight:700">⚠️ Domain not authorized. Official app link use karo.</span>';
      toast('Domain not authorized', 'err');
    } else {
      /* Fallback: try redirect */
      setTimeout(function() { auth.signInWithRedirect(gp).catch(function(e2){ toast(e2.message||'Login failed','err'); }); }, 300);
    }
  });
}
...
function boot() {
  if (!U) return;
  /* Double-boot prevention — fixes-v8.js bhi yahi karta hai as backup */
  if (window._bootCalled) { console.warn('[Boot] Already called — skip'); return; }
  ...
  db.ref('users/' + U.uid).on('value', function(s) {
    ...
    console.error('[boot] user data error:', err && err.message);
    var hl = document.getElementById('homeList');
    if (hl) hl.innerHTML = '<div style="text-align:center;padding:40px 20px"><div style="font-size:48px;margin-bottom:12px">⚠️</div><div style="font-size:15px;font-weight:700;color:#ff6b6b;margin-bottom:8px">Data load error</div><div style="font-size:12px;color:#aaa;margin-bottom:16px">Internet check karo aur refresh karo</div><button onclick="location.reload()" style="padding:12px 24px;border-radius:12px;background:rgba(0,212,255,.15);border:1px solid rgba(0,212,255,.3);color:#00d4ff;font-weight:700;cursor:pointer">🔄 Refresh</button></div>';
  });
  ...
  }, function(err) { console.warn('[boot] matches error:', err && err.message); renderHome(); });
  ...
  }, function(err) { console.warn('[boot] tournaments error:', err && err.message); });
...
function cJoin(id) {
  ...
  var prizeLabel = isCoin ? '💚 Green Diamond (rank/badge ke liye)' : isSkyDia ? '💚 Green Diamond (rank/badge ke liye)' : '🪙 Coins';
...
function renderWallet() {
  ...
  if (gdc) {
    var greenDia = Math.max(Number((UD.realMoney||{}).winnings)||0, 0) + Math.max(Number((UD.realMoney||{}).bonus)||0, 0);
    gdc.textContent = '💚 ' + greenDia;
  }
...
function renderRank(tab) {
  ...
  var rankHeader = '<div class="rank-formula-banner" onclick="window.showHowRankWorks&&showHowRankWorks()">' +
    '📊 Rank Points = Wins×40 + Kills×2 + Matches×1 + WinStreak×10 &nbsp;<span style="font-size:10px;opacity:.7">→ Tap for details</span>' +
    '</div>';
  ...
  db.ref('users').limitToLast(300).once('value', function(snap) {
    var users = [];
    ...
    users.sort(function(a, b) {
      var getVal = function(u) {
        ...
        // rankpoints: Wins×40 + Kills×2 + Matches×1 + WinStreak×10
        var wins = Number(st.wins||0), kills = Number(st.kills||0),
            matches = Number(st.matches||0), streak = Number(st.winStreak||u.winStreak||0);
        return wins*40 + kills*2 + matches*1 + streak*10;
      };
...
function _renderRankList(rc, users, sortKey, tabBar, seasonBanner, isSeason) {
  ...
  for (var j = 3; j < users.length; j++) {
    ...
    h += '<div class="rank-info"><div class="rn">' + (u.ign || u.displayName || 'Player') + (isMe ? ' <span style="font-size:9px;color:var(--green)">YOU</span>' : '');
...
function renderProfile() {
  ...
  /* ── Achievements button ── */
  h += '<button class="prof-btn" onclick="window.showAchievements&&showAchievements()" style="background:linear-gradient(135deg,rgba(255,215,0,.1),rgba(255,140,0,.06));border:1px solid rgba(255,215,0,.2);color:#ffd700;margin-bottom:8px">';
