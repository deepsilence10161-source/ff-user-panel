...
window.doEmailRegisterSupabase = async function(name, email, pass) {
  ...
  try {
    ...
    if (result && result.user) {
      /* Issue #4 Fix: Guard profile creation — rollback auth on failure so
         user is never left with auth credentials but no DB profile row. */
      var profile = null;
      try { profile = await DB.users.create(result.user.id, { ign: name, email: email }); }
      catch(profileErr) { console.error('[Auth] Profile create error:', profileErr); }
      if (!profile) {
        try { await DB.auth.logout(); } catch(e) {}
        toast('❌ Account creation failed. Please try again.', 'err');
        return;
      }
      closeModal();
...
function doLogout() {
  /* ── Bug 41 Fix: Destroy all Firebase listeners via LM ── */
  try { if (window.LM && window.LM.destroyAll) window.LM.destroyAll(); } catch(e) {}
  /* ── Bug 3 Fix: Clear all db-bridge polling intervals to prevent memory leak ── */
  if (window._bridgePolls) {
    Object.keys(window._bridgePolls).forEach(function(key) {
      clearInterval(window._bridgePolls[key]);
    });
    window._bridgePolls = {};
    console.log('[Bridge] All polling intervals cleared on logout');
  }
  ...
  window._allJoinChecksDone = false;
  UD = null; U = null; MT = {}; JR = {}; NOTIFS = []; WH = []; REFS = []; TXNS = []; _READ_KEYS = {};

  /* L2 Fix: Reset notification badge on logout — prevents stale count re-appearing
     on next login before fresh unread count is fetched from Supabase. */
  window._unreadNotifCount = 0;
  var _bellBadge = document.querySelector('.notif-badge, #notifBadge, [data-badge="notif"]');
  if (_bellBadge) { _bellBadge.textContent = ''; _bellBadge.style.display = 'none'; }

  /* Bug 15 Fix: Clear offline join queue on logout — prevents queued operations
     from a previous user being replayed when a different user logs in on same device. */
  if (window.OQ && typeof window.OQ.clearAll === 'function') {
    window.OQ.clearAll();
  } else {
    /* Direct IndexedDB clear if OQ not available */
    try {
      var _idbReq = indexedDB.open('mes_offline_q', 2);
      _idbReq.onsuccess = function(e) {
        var _idb = e.target.result;
        if (_idb.objectStoreNames.contains('ops')) {
          var _tx = _idb.transaction('ops', 'readwrite');
          _tx.objectStore('ops').clear();
          console.log('[Logout] Offline queue cleared ✅');
        }
        _idb.close();
      };
    } catch(e) { console.warn('[Logout] IDB clear failed:', e); }
  }
...
/* ── doGoogleLogin alias — index.html onclick calls this ── */
window.doGoogleLogin = function() {
  if (window._supaGoogleLogin) {
    window._supaGoogleLogin();
  } else if (window._firebaseGoogleLogin) {
    window._firebaseGoogleLogin();
  } else {
    console.error('[Auth] No Google login method available');
  }
};

/* ═══════════════════════════════════════════════════════════════
   Cross-cutting #5 Fix: 30-day session timeout
   Auto-logout if user hasn't been active for 30 days.
   Last-activity timestamp stored in localStorage (device-local).
═══════════════════════════════════════════════════════════════ */
(function() {
  var SESSION_MAX_MS = 30 * 24 * 60 * 60 * 1000; // 30 days
  var ACT_KEY        = '_mesLastActivity';

  function _touchActivity() {
    try { localStorage.setItem(ACT_KEY, Date.now().toString()); } catch(e) {}
  }

  function _checkSessionAge() {
    if (!window.U) return; // not logged in
    try {
      var last = parseInt(localStorage.getItem(ACT_KEY) || '0');
      if (!last) { _touchActivity(); return; } // first time — set baseline
      if (Date.now() - last > SESSION_MAX_MS) {
        console.warn('[Auth] Session expired after 30 days of inactivity — logging out');
        if (window.toast) window.toast('Session expired. Please log in again.', 'err');
        setTimeout(function() {
          if (window.doLogout) window.doLogout();
          else { try { window.auth && window.auth.signOut(); } catch(e) {} }
        }, 1500);
      }
    } catch(e) {}
  }
  ...
  /* Check on app boot (after UD is loaded) */
  var _checkDone = false;
  var _checkIv = setInterval(function() {
    if (window.U && !_checkDone) {
      _checkDone = true;
      clearInterval(_checkIv);
      _touchActivity(); // mark login as activity
      _checkSessionAge();
    }
  }, 2000);
})();
