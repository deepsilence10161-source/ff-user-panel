...
window.doGoogleLogin = function() {
  ...
  function _tryLogin() {
    ...
    if (_isWebView()) {
      /* Android WebView mein signInWithPopup broken hai:
         Popup ek alag WebView mein khulta hai jiska IndexedDB ISOLATED hota hai.
         Firebase auth token popup ke IndexedDB mein save hota hai,
         main WebView ko kabhi nahi milta → onAuthStateChanged NEVER fires → spinner stuck.

         FIX: WebView mein popup skip karo, seedha signInWithRedirect use karo.
         Redirect flow main WebView navigate karta hai → wapas aane pe
         getRedirectResult() token le leta hai → onAuthStateChanged fires ✅ */
      ...
      fbAuth.setPersistence(persist)
        .then(function() {
          ...
          return fbAuth.signInWithRedirect(fbGp);
        })
        .catch(function(e) {
