  ...
  function _tryLogin() {
    ...
    if (_isWebView()) {
      /* ── Chrome Custom Tab Flow ──────────────────────────────
         Flow:
         1. signInWithRedirect call hota hai
         2. Firebase accounts.google.com pe jaana chahta hai
         3. MainActivity.shouldOverrideUrlLoading intercept karta hai
            accounts.google.com URL ko → Chrome Custom Tab mein open karta hai
         4. Chrome mein user ke SAVED Google accounts dikh'te hain ✅
         5. User select karta hai → Firebase redirect → github.io Custom Tab mein
         6. github.io load → getRedirectResult() → token
         7. JS: onUserLoggedIn() signal → Custom Tab band → app foreground ✅ */
