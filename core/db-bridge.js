...
(function() {
  ...
  function _supaWrite(path, value, isUpdate) {
    ...
    if (root === 'users' && parts[1]) {
      ...
      /* users/{uid}/coins */
      if (field === 'coins') {
        /* ✅ SECURITY FIX (2026-07-17, CRITICAL): coins/sky_diamonds/
           green_diamonds are no longer directly client-UPDATE-able at
           all (see the grant-block comment above for the full
           explanation — this was a severe balance-tampering hole). This
           bridge path previously did a direct absolute-value SET
           (`coins: value`), which is fundamentally the wrong operation
           anyway even setting security aside — every legitimate caller of
           this path is trying to ADD to the balance (e.g. after an ad
           watch, a match win), not overwrite it outright, and a raw SET
           is exactly what a race condition between two concurrent credits
           would corrupt. Routed through increment_balance instead, which
           is atomic and now identity-checked. If a caller genuinely needs
           to set an absolute value (e.g. an admin correcting a balance),
           that must go through a dedicated admin RPC, not this path. */
        var coinsDelta = typeof value === 'number' ? value : parseInt(value) || 0;
        console.warn('[Bridge] users/coins now routes through increment_balance (delta:', coinsDelta, ') instead of a direct SET — if this call site intended an absolute value, not a delta, it needs a dedicated admin RPC instead.');
        return window._supa.rpc('increment_balance', { p_uid: targetUid, p_col: 'coins', p_amount: coinsDelta });
      }
      /* users/{uid}/realMoney/deposited|winnings|bonus */
      if (field === 'realMoney') {
        var subField = parts[3];
        /* ✅ SECURITY FIX (2026-07-17, CRITICAL): same fix as 'coins' above
           — sky_diamonds/green_diamonds are no longer directly
           client-UPDATE-able. Also same Math.max(0,...) absolute-SET
           pattern replaced with increment_balance (positive amounts only
           — increment_balance itself rejects negative amounts, so this
           also can't be used to zero out or reduce a balance; use
           decrement_balance for that, as diamond-system.js's withdrawal
           flow now correctly does). */
        if (subField === 'deposited') {
          var depositAmt = Math.max(0, parseInt(value) || 0);
          return window._supa.rpc('increment_balance', { p_uid: targetUid, p_col: 'sky_diamonds', p_amount: depositAmt });
        }
        if (subField === 'winnings') {
          var winAmt = Math.max(0, parseInt(value) || 0);
          return window._supa.rpc('increment_balance', { p_uid: targetUid, p_col: 'green_diamonds', p_amount: winAmt });
        }
      }
      /* users/{uid}/greenDiamonds */
      if (field === 'greenDiamonds') {
        var gdAmt = Math.max(0, parseInt(value) || 0);
        return window._supa.rpc('increment_balance', { p_uid: targetUid, p_col: 'green_diamonds', p_amount: gdAmt });
      }
      /* users/{uid}/skyDiamonds */
      if (field === 'skyDiamonds') {
        var sdAmt = Math.max(0, parseInt(value) || 0);
        return window._supa.rpc('increment_balance', { p_uid: targetUid, p_col: 'sky_diamonds', p_amount: sdAmt });
      }
    ...
    if (root === 'matches' && parts[1] && parts[2] === 'joinedSlots') {
      return window._supa.rpc('increment_balance', { p_uid: parts[1], p_col: 'filled_slots', p_amount: 1 }).then(null, function(){});
    }
