...
function afterLogin(user) {
  ...
  function _doFirebaseLoad() {
    /* v3.0: Firebase RTDB fallback — creates minimal UD and boots */
    clearTimeout(_bootTimer);
    window.UD = { uid: user.uid, ign: user.displayName || user.email || 'Player', email: user.email || '', profileImage: user.photoURL || '', coins: 0, skyDiamonds: 0, greenDiamonds: 0, realMoney: { deposited: 0, winnings: 0, bonus: 0 }, stats: { matches: 0, wins: 0, kills: 0, earnings: 0 }, profileStatus: 'not_requested' };
    if (!_bootDone) { _bootDone = true; boot(); }
  }
