    ...
    DB.users.getMe().then(function(profile) {
      ...
      if (profile === null) {
        ...
        /* getMe() returns undefined when the fetch itself failed (network/
           DB error) — NOT the same as "no profile exists". Do not create a
           duplicate user or re-run onboarding; just render with whatever
           window.UD already has (e.g. from a previous session) and let the
           user retry naturally on their next action. */
        _refreshUI();
      } else {
        window.UD = profile;
