...
function _findUserByFF(ffUid, callback) {
  ...
  window._supa.from('users').select('id,ign,ff_uid,avatar_url,profile_status,coins,sky_diamonds,total_matches,total_wins,total_kills')
...
function _findUserByIGN(ign, callback) {
  ...
  window._supa.from('users').select('id,ign,ff_uid,avatar_url,profile_status').eq('ign', ign).limit(1).maybeSingle()
