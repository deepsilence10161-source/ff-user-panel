...
function _cleanupChannels() {
  _realtimeChannels.forEach(function(ch) { try { window._supa.removeChannel(ch); } catch(e) {} });
  _realtimeChannels = [];
...
function _applyUser(sp) {
  ...
  UD.profileStatus = sp.profileStatus || UD.profileStatus || 'not_requested';
  ...
  if (sp.premium_tier) {
    ...
    UD.premium.tier = ['silver','gold','diamond'].indexOf(sp.premium_tier) + 1;
    UD.premium.expiresAt = sp.premium_expires_at ? new Date(sp.premium_expires_at).getTime() : 0;
  }
...
function _bootMatches() {
  ...
  _rtCh('matches-all', 'matches', null, function(p) {
    ...
    } else if (p.eventType === 'DELETE' && p.old) { delete MT[p.old.id]; renderHome(); renderSP(); renderMM(); }
  });
...
function _toJR(jr) {
  return { _key: jr.id, id: jr.id, matchId: jr.match_id, tournamentId: jr.match_id, userId: jr.user_id, uid: jr.user_id, status: jr.status||'pending', entryType: jr.entry_type==='coins'?'coin':'paid', entryFee: jr.entry_fee_paid||0, userName: jr.ign_at_join||'', ign: jr.ign_at_join||'', kills: jr.kills||0, rank: jr.placement||0, winnings: jr.prize_earned||0, inRoom: jr.in_room||false, checkedIn: jr.checked_in||false, refunded: jr.status==='refunded', resultStatus: jr.status==='approved'?'done':jr.status, createdAt: jr.created_at ? new Date(jr.created_at).getTime() : 0 };
}
...
function _bootWallet() {
  ...
  _rtCh('wallet-' + U.uid, 'sd_requests', 'user_id=eq.' + U.uid, function(p) {
    if (p.new) {
      var ns=(p.new.status||'').toLowerCase(), os=p.old?(p.old.status||'').toLowerCase():'';
      if (os && os!==ns) { if (ns==='approved') toast('✅ Deposit 💎'+(p.new.sd_amount||0)+' added!','ok'); else if (ns==='rejected') toast('❌ Deposit rejected','err'); }
      _loadWalletHistory();
    }
  });
...
function _loadTransactions() {
  ...
  window._supa.from('wallet_transactions').select('*').eq('user_id', U.uid).order('created_at', { ascending: false }).limit(50)
    .then(function(r) {
      TXNS = (r.data||[]).map(function(t) { return { _key:t.id, type:t.txn_type==='credit'?'credit':'debit', amount:t.txn_type==='credit'?t.amount:-t.amount, description:t.note||t.reason||'', currency:t.currency||'coins', timestamp:t.created_at?new Date(t.created_at).getTime():0, read:true }; });
      if (curScr==='wallet') renderWallet();
...
function _startMatchTimers() {
  ...
  setInterval(function() {
    ...
    for (var mid in MT) {
      ...
      else if (es==='completed') { el.style.color='var(--text-muted)'; el.textContent='Ended'; }
