    ...
    users: {
      ...
      getMe: async function() {
        ...
        var { data, error } = await window._supa
          .from('users')
          .select('id,ign,email,avatar_url,coins,sky_diamonds,green_diamonds,rank_tier,rank_points,total_wins,total_kills,total_matches,city,is_creator,premium_level,clan_id,referral_code,profile_status,is_banned,ban_reason,accepted_policy,created_at')
          .eq('id', uid)
          .maybeSingle();
    ...
    battlePass: {
      ...
      claimTier: async function(seasonId, tier, rewardCoins) {
        ...
        await window._supa
          .from('battle_pass_progress')
          .upsert({
            user_id: uid, season_key: seasonId,
