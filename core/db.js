  ...
  window.DB = {
    ...
    auth: {
      ...
      loginWithEmail: async function(email, password) {
        var { data, error } = await window._supa.auth.signInWithPassword({
          email: email, password: password
        });
        if (error) return _err('auth.loginWithEmail', error);
        return data;
      },

      /* Email + Password register */
      registerWithEmail: async function(email, password, name) {
        var { data, error } = await window._supa.auth.signUp({
          email: email,
          password: password,
      ...
      logout: async function() {
        var { error } = await window._supa.auth.signOut();
        if (error) _err('auth.logout', error);
