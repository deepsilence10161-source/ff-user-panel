  ...
  window.DB = {
    ...
    auth: {
      ...
      loginWithGoogle: async function() {
        var _s = window._supa;
        var { data, error } = await _s.auth.signInWithOAuth({
          provider: 'google',
          options: {
        ...
        if (error) return _err('auth.loginWithGoogle', error);
        return data;
      },

      /* Email + Password login — uses Firebase (source of truth for auth)
         Supabase Firebase integration automatically accepts Firebase JWT tokens,
         so we never call supabase.auth.signInWithPassword() directly. */
      loginWithEmail: async function(email, password) {
        var firebaseAuth = window.auth || (window.firebase && window.firebase.auth());
        if (!firebaseAuth) return _err('auth.loginWithEmail', new Error('Firebase auth not ready'));
        try {
          var result = await firebaseAuth.signInWithEmailAndPassword(email, password);
          /* Sync Firebase JWT to Supabase session so RLS works */
          if (window._supa && result.user) {
            var token = await result.user.getIdToken();
            await window._supa.auth.signInWithCustomToken(token).catch(function(){});
          }
          return { user: { id: result.user.uid, email: result.user.email } };
        } catch(e) {
          var msg = e.code === 'auth/user-not-found'   ? 'User nahi mila. Pehle register karo.' :
                    e.code === 'auth/wrong-password'    ? 'Password galat hai.' :
                    e.code === 'auth/invalid-email'     ? 'Email sahi format mein daalo.' :
                    e.code === 'auth/too-many-requests' ? 'Bahut zyada tries. Thodi der baad try karo.' :
                    e.message;
          return _err('auth.loginWithEmail', new Error(msg));
        }
      },

      /* Email + Password register — Firebase mein account banao */
      registerWithEmail: async function(email, password, name) {
        var firebaseAuth = window.auth || (window.firebase && window.firebase.auth());
        if (!firebaseAuth) return _err('auth.registerWithEmail', new Error('Firebase auth not ready'));
        try {
          var result = await firebaseAuth.createUserWithEmailAndPassword(email, password);
          /* Update display name in Firebase */
          if (result.user && name) {
            await result.user.updateProfile({ displayName: name }).catch(function(){});
          }
          /* Sync to Supabase */
          if (window._supa && result.user) {
            var token = await result.user.getIdToken();
          ...
          var msg = e.code === 'auth/email-already-in-use' ? 'Yeh email pehle se registered hai. Login karo.' :
                    e.code === 'auth/weak-password'         ? 'Password kam se kam 6 characters ka hona chahiye.' :
                    e.code === 'auth/invalid-email'         ? 'Email sahi format mein daalo.' :
                    e.message;
          return _err('auth.registerWithEmail', new Error(msg));
        }
      },

      /* Logout — Firebase se sign out karo (Supabase session auto-expires) */
      logout: async function() {
        var firebaseAuth = window.auth || (window.firebase && window.firebase.auth());
        try { if (firebaseAuth) await firebaseAuth.signOut(); } catch(e) {}
        try { await window._supa.auth.signOut(); } catch(e) {}
    ...
    users: {
      ...
      getMe: async function() {
        ...
        var { data, error } = await window._supa
          ...
          .eq('id', uid)
          .single();
        if (error) return _err('users.getMe', error);
        return data;
      },
      ...
      create: async function(uid, profile) {
        ...
        if (error) return _err('users.create', error);
        return data;
      },
      ...
      update: async function(fields) {
        ...
        var { data, error } = await window._supa
          ...
          .eq('id', uid);
        if (error) return _err('users.update', error);
        return data;
      },
