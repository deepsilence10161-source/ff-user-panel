/* ====== MODAL ====== */
/* Bug #32 Fix: Proper pushState/popstate for modal back button handling.
   Previously: replaceState on close removed the page's own history entry,
   causing back button to exit app instead of closing modal. */
var _modalHistoryPushed = false;

function openModal(title, html) {
  /* Push a modal state so back button closes it, not exits app */
...
function closeModal() {
  ...
  if (_modalHistoryPushed) {
    ...
    /* Use history.back() only if current state is our modal state */
...
/* Listen for popstate (back button) — close modal instead of navigating */
