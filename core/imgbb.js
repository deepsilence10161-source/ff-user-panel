...
(function() {
  ...
  /* BUG FIX (2026-08): "Login required to upload" even when user IS
     logged in (name/UID visible on screen). Root cause: firebase.auth()
     .currentUser can be momentarily null right after page load / tab
     switch, before Firebase has finished restoring the session from
     IndexedDB — that restore is async even though the user was already
     signed in. The old code checked currentUser synchronously and gave
     up instantly if it wasn't there yet, with zero retry.
     Fix: if currentUser isn't there yet, wait (with a short timeout) for
     onAuthStateChanged to fire once via a one-time listener, instead of
     failing immediately. */
  function _getAuthToken(cb) {
    try {
      ...
      var unsub = firebase.auth().onAuthStateChanged(function(user) {
        ...
        try { unsub(); } catch (e) {}
        if (user) {
          user.getIdToken().then(cb).catch(function() { cb(null); });
        } else {
          cb(null);
        }
      });
