...
(function() {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
      ...
      (function _registerSW(attempt) {
        /* BUG FIX (2026-07): was hardcoded to '/sw.js' scope '/', which
           only exists on the Cloudflare Workers (root) domain — it 404'd
           on GitHub Pages (/ff-user-panel/ subpath). Relative path works
  ...
  function _handleOnline() {
    ...
    if (window.DB) {
      DB.config.load();
      /* ✅ BUG FIX (2026-08): "Green Diamond shows 35 in wallet but 0 in
         header chip" — this handler was doing `window.UD = u` with the
         RAW Supabase row from getMe(). That row has snake_case columns
         (green_diamonds, sky_diamonds, ...) but every screen (header.js,
         wallet.js) reads UD.greenDiamonds / UD.skyDiamonds (camelCase) —
         those are only ever populated by _applyUser() in listeners.js,
         which does the snake→camel mapping. A hard `UD = u` here wiped
         out every camelCase field the moment the device reconnected to
         the network (common on the mobile data this app runs on), so
         the header chip silently went to 0 while the DB value (35) was
         completely fine — same money, just unreadable field name. Fix:
         route through _applyUser() (a MERGE, snake→camel, same function
         every other load path uses) instead of replacing UD outright. */
      if (window.U) DB.users.getMe().then(function(u) {
        if (u) {
          if (window._applyUser) { _applyUser(u); }
          else { window.UD = window.UD || {}; Object.assign(window.UD, u); }
  ...
  /* ── BUG FIX (2026-07): report #mainContent's real scroll position to
     the Android bridge, so native pull-to-refresh (MainActivity.java's
     setOnChildScrollUpCallback) can tell "actually at the top of the
     page" apart from "just swiping up while scrolling a long list".
     Only calls the bridge when the at-top STATE actually flips, not on
     every scroll pixel, so this stays cheap. No-ops in a normal browser
     (window.Android only exists inside the Android app's WebView). */
  (function() {
    var _wasAtTop = true;
    function _reportScroll() {
      if (!window.Android || typeof window.Android.reportScrollTop !== 'function') return;
      var el = document.getElementById('mainContent');
      var atTop = !el || el.scrollTop <= 0;
      if (atTop !== _wasAtTop) {
        _wasAtTop = atTop;
        try { window.Android.reportScrollTop(atTop); } catch (e) {}
      }
    }
    document.addEventListener('DOMContentLoaded', function() {
      var el = document.getElementById('mainContent');
      if (el) el.addEventListener('scroll', _reportScroll, { passive: true });
      /* Modals scroll independently too — cover the common one as well */
      document.addEventListener('scroll', function(ev) {
        if (ev.target && ev.target.classList && ev.target.classList.contains('modal-body')) _reportScroll();
