...
window.onAdReward = function() {
  ...
  if (window._supa && window.U) {
    window._supa.rpc('increment_balance', { p_uid: window.U.uid, p_col: 'coins', p_amount: adCoins }).catch(function(){});
    window._supa.from('wallet_transactions').insert({ user_id: window.U.uid, currency: 'coins', txn_type: 'credit', amount: adCoins, reason: 'ad_reward' }).catch(function(){});
  }
...
function confirmAdMatchJoin(matchId) {
  ...
  window._supa.from('join_requests').insert({
    ...
    if (window._supa) {
      window._supa.rpc('increment_match_slots', { p_match_id: matchId }).catch(function(){});
    }
...
window.updateSeasonStats = function(uid, statsUpdate) {
  ...
  if (window._supa) {
    if (wins    > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_wins',    p_amount: wins    }).catch(function(){});
    if (kills   > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_kills',   p_amount: kills   }).catch(function(){});
    if (matches > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_matches', p_amount: matches }).catch(function(){});
