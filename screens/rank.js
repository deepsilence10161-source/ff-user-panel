...
window.onAdReward = function() {
  ...
  db.ref('users/' + U.uid + '/coins').transaction(function(c) { return (c || 0) + adCoins; });
  if (window._supa && U) {
    window._supa.rpc('increment_balance', { p_uid: U.uid, p_col: 'coins', p_amount: adCoins }).catch(function(){});
    window._supa.from('wallet_transactions').insert({ user_id: U.uid, currency: 'coins', txn_type: 'credit', amount: adCoins, reason: 'ad_reward' }).catch(function(){});
  }
  if (UD) { UD.coins = (UD.coins || 0) + adCoins; if (window.updateHdr) updateHdr(); }
  toast('+' + adCoins + ' Coins earned! 🪙', 'ok');
};
...
window.onAdReward = function() {
  if (window._adMatchPending) {
    ...
    if (window.onAdReward) onAdReward();
...
window.updateSeasonStats = function(uid, statsUpdate) {
  ...
  if (window._supa) {
    if ((statsUpdate.wins || 0) > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_wins', p_amount: statsUpdate.wins }).catch(function(){});
    if ((statsUpdate.kills || 0) > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_kills', p_amount: statsUpdate.kills }).catch(function(){});
    if ((statsUpdate.matches || 0) > 0) window._supa.rpc('increment_balance', { p_uid: uid, p_col: 'total_matches', p_amount: statsUpdate.matches }).catch(function(){});
    var pts = window.calcRkScore ? window.calcRkScore(statsUpdate) : 0;
    if (pts > 0) window._supa.rpc('increment_rank_points', { p_uid: uid, p_points: pts }).catch(function(){});
  }
...
window.assignTop3Badges = function(users) {
  ...
  users.slice(0, 3).forEach(function(u, i) {
    ...
    /* Stats via Supabase */
    if (window._supa && u._uid) { var _c = key==='wins'?'total_wins':key==='kills'?'total_kills':'total_matches'; window._supa.rpc('increment_balance',{p_uid:u._uid,p_col:_c,p_amount:1}).catch(function(){}); }
  });
};
