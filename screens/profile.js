...
function renderProfile() {
  ...
  h += '<div class="gi-row"><span class="gi-l">IGN</span><span class="gi-v">' + (window.escHtml?window.escHtml(UD.ign||'-'):(UD.ign||'-')) + '</span></div>';
...
function _doSubmitProfileRequest(ign, uid, phone, btn) {
    ...
    window._supa.from('profile_requests').upsert({
      ...
      toast('Submit failed — retry karo', 'err');
      _profileUpdateSubmitting = false;
      if (btn) { btn.disabled = false; btn.textContent = 'Submit for Verification'; }
    });
}
...
window.showProfileSettings = function() {
  ...
  var items = [
    ...
    { icon:'fa-copy',          label:'My Referral Code',      color:'#00ff9c', fn:'if(window.showReferral)showReferral();window.closeProfileSettings()' },
