...
function renderProfile() {
  ...
  /* Banner area with change button
     BUG FIX (2026-07): banner was only 110px tall with an 86px avatar —
     much smaller/flatter than the reference screenshot, which shows a
     tall, prominent banner with a big centered avatar. Bumped both up
     (banner 110→190px, avatar 86→112px) keeping the same overlap style. */
  h += '<div id="profBannerWrap" style="position:relative;width:100%;height:190px;' + bannerStyle + ';border-radius:16px 16px 0 0;overflow:hidden">';
  ...
  h += '<div style="display:flex;flex-direction:column;align-items:center;margin-top:-58px;position:relative;z-index:2;padding-bottom:14px;background:linear-gradient(180deg,transparent 0%,rgba(5,5,7,.95) 70px)">';
  h += '<div class="prof-ava-wrap"><div class="prof-ava" style="width:112px;height:112px;font-size:40px;border:3px solid ' + rk.color + ';box-shadow:0 0 20px ' + rk.color + '88,0 0 0 3px rgba(5,5,7,.8);' + ringAnim + '">' + av + '</div>';
  ...
  h += '<div style="font-size:20px;font-weight:900;margin-top:8px;text-align:center">' + (window.escHtml?window.escHtml(UD.ign||UD.displayName||'Player'):(UD.ign||UD.displayName||'Player')) + premBadge + '</div>';
...
function uploadProfImg(inp) {
  ...
  if (window.uploadProfileImage) { uploadProfileImage(inp.files[0], function(url) { if (url) { toast('Photo updated! ✅', 'ok'); setTimeout(renderProfile, 300); } }); return; }
...
function _doSubmitProfileRequest(ign, uid, phone, btn) {
    ...
    window._supa.from('profile_requests').upsert({
      ...
      window._supa.from('users').update(userUpdate).eq('id', U.uid).catch(function(){});
      /* Update local UD */
      if (window.UD) {
  ...
  window._supa.from('vouchers').select('*').eq('code', code).single()
    .then(function(r) {
      ...
      window._supa.rpc('increment_balance', { p_uid: window.U.uid, p_col: _col, p_amount: ra })
        .then(function() {
          window._supa.from('vouchers').update({ used_count: (v.used_count || 0) + 1 }).eq('code', code).catch(function(){});
          window._supa.from('wallet_transactions').insert({ user_id: window.U.uid, currency: _col, txn_type: 'credit', amount: ra, reason: 'voucher', ref_id: code }).catch(function(){});
