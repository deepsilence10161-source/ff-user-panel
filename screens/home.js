...
function liveSearchUsers(q) {
  ...
  _searchTimer = setTimeout(function() {
    ...
    window._supa.from('users')
      .select('id,ign,ff_uid,avatar_url,profileStatus,city,total_matches,total_wins,total_kills')
      .or('ign.ilike.%' + q + '%,ff_uid.ilike.%' + q + '%')
      .eq('is_banned', false).limit(10)
      .then(function(r) {
        var results = (r.data || []).map(function(u) {
          return { uid: u.id, u: { ign: u.ign||'', displayName: u.ign||'', ffUid: u.ff_uid||'', profileImage: u.avatar_url||'', profileStatus: u.profileStatus||'', city: u.city||'', stats: { matches: u.total_matches||0, wins: u.total_wins||0, kills: u.total_kills||0 } } };
        });
...
function viewPublicProfile(uid) {
  ...
  window._supa.from('users').select('id,ign,ff_uid,avatar_url,bio,total_matches,total_wins,total_kills').eq('id', uid).single()
    .then(function(r) {
      ...
      h += '<div style="font-size:20px;font-weight:900">' + (u.ign || 'Player') + '</div>';
