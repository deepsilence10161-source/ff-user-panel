...
function checkRefunds() {
  for (var k in JR) {
    ...
    if (st === 'cancelled' || st === 'canceled') {
      ...
      if (_isCoin) db.ref('users/' + U.uid + '/coins').transaction(function(c) { return (c || 0) + fee; });
      ...
      if (window._supa && U) {
        ...
        window._supa.rpc('increment_balance', { p_uid: U.uid, p_col: _refCol, p_amount: fee }).catch(function(){});
        window._supa.from('wallet_transactions').insert({ user_id: U.uid, currency: _refCol, txn_type: 'credit', amount: fee, reason: 'match_refund', ref_id: jr.matchId }).catch(function(){});
      }
