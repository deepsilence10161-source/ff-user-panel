      ...
      if (window._supa && U) {
        ...
        window._supa.rpc('increment_balance', { p_uid: U.uid, p_col: _refCol, p_amount: _jrFee })
          .then(function(res) {
            ...
            window._supa.from('join_requests').update({ status: 'refunded' }).eq('id', _jrKey).catch(function(){});
...
function confirmInRoom(jKey, btn) {
  ...
  if (!window._supa) { btn.disabled = false; toast('Service unavailable', 'err'); return; }
  window._supa.from('join_requests')
