...
function renderWallet() {
  ...
  // the circular icon badge above each number (see index.html). Also now
  // updating wBreak2, the matching breakdown line under the Coins card.
  var wt = $('wTotal'), wb = $('wBreak'), wc = $('wCoins'), wb2 = $('wBreak2');
  if (wt) { wt.textContent = skyDia; wt.style.color = '#00d4ff'; }
  var _breakText = 'Sky Diamonds: 💎' + skyDia + ' | Coins: 🪙' + coins;
  if (wb) wb.innerHTML = _breakText;
  if (wb2) wb2.innerHTML = _breakText;
...
function submitSponsoredWd(maxBal) {
  ...
  if (!window._supa) { toast('Service unavailable. Try again.', 'err'); return; }
  ...
  Promise.all([
    ...
    function saveRequest(screenshotUrl) {
      window._supa.from('sd_requests').insert({
        ...
        sd_amount: wfAmt,
        upi_ref: utr,
        payment_proof: screenshotUrl || null,
        img_hash: _imgHash,
        status: 'pending'
      }).then(function() {
