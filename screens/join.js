...
async function _doJoinCore(id, t, tp) {
  ...
  var _jifTimer = setTimeout(function() { _joinInFlight = false; }, 8000);
    ...
    if (_captainFee > 0) {
      ...
      window._supa.rpc('validate_and_join_match', {
        ...
        db.ref('joinRequests/' + jid).set(Object.assign({ requestId: jid, userId: U.uid, matchId: id,
          entryFee: _captainFee, entryType: isCoin ? 'coin' : 'money', status: 'joined', createdAt: Date.now() }, _joinData));
    ...
    var _makePartnerJR = function(pUid, pName, pFFUid, pIndex) {
      ...
      if (_feeType === 'each_pays' && pEntryFee > 0) {
        if (isCoin) {
          ...
          if (window._supa) {
            window._supa.rpc('decrement_balance', { p_uid: pUid, p_col: 'coins', p_amount: pEntryFee }).catch(function(){});
          ...
          if (window._supa) {
            window._supa.rpc('decrement_balance', { p_uid: pUid, p_col: 'sky_diamonds', p_amount: pEntryFee }).catch(function(){});
            window._supa.from('wallet_transactions').insert({
      ...
      if (window._supa) {
        window._supa.from('users').select('is_banned').eq('id', pUid).single()
          .then(function(r) {
